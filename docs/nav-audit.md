...
| NAV-07 | `BP` | 🟠 | FE `admin/stranky` = PJ(5) ↔ BE pages write = PomocnyPJ(4) | nesoulad: BE `assertCanWrite` ([pages.service.ts:826](../../Projekt-ikaros/backend/src/modules/pages/pages.service.ts#L826)) = `role >= PomocnyPJ`, FE admin tabulka = PJ → **PomocnyPJ může CRUD stránek přes API**, ač FE správu stránek ukazuje až PJ. Rozhodnout, která strana je kanonická (BE doc přiznává „PomocnyPJ+"). | L1 agent (BE) | ⏭️ flag — rozhodnout kanonickou roli (PomocnyPJ vs PJ) |
| NAV-08 | `BP` | 🟠 | FE `admin/kalendare` = PJ(5) ↔ BE calendar-config write = PomocnyPJ(4) | tentýž vzor jako NAV-07: `assertCanWrite` ([world-calendar-config.service.ts:356](../../Projekt-ikaros/backend/src/modules/world-calendar-config/world-calendar-config.service.ts#L356)) = PomocnyPJ → PomocnyPJ může multi-config kalendáře CRUD přes API, FE až PJ. | L1 agent (BE) | ⏭️ flag — rozhodnout kanonickou roli (PomocnyPJ vs PJ) |
| NAV-09 | `BP`/`IS` | 🟠 | FE `admin/headline`/`nastaveni` ↔ BE `GET worlds/:id/settings` | side-finding: `getSettings` ([worlds.service.ts:959](../../Projekt-ikaros/backend/src/modules/worlds/worlds.service.ts#L959)) **nemá member check** → jakýkoli přihlášený přečte `world_settings` **libovolného** světa (headline, nav config, AKJ úrovně) cross-tenant. Mutace settings je správně PJ-only. Read leak (cross-svět). | L2 ověřeno (kód) | ⏭️ flag — fix = filtrovaný read (anon/veřejný svět možná čte motiv) |
...
**BE (ověřeno: campaign jest 52/52 + eslint + prettier):**
- **NAV-06** — [campaign.service.ts `getWorldRole`](../../Projekt-ikaros/backend/src/modules/campaign/campaign.service.ts) už **nefalbackuje nečlena na Hrac** → nečlen dostane `ForbiddenException` (zavírá cross-tenant write hole napříč storyboard/pavučina/obchod). Test upraven na nové chování.

**Flagnuté (čekají rozhodnutí — neopraveno):**
- **NAV-07 / NAV-08** — rozhodnout kanonickou roli: BE write = PomocnyPJ vs FE admin route = PJ (pages, calendar-config). Sjednotit jednu stranu.
- **NAV-09** — `GET worlds/:id/settings` bez member checku (cross-tenant read). Fix není one-liner: anon/veřejný svět možná čte motiv ze settings → potřeba **filtrovaný read** (theme veřejně, zbytek member-only), ne slepý gate.
