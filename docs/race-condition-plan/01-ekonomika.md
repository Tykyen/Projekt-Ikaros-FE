...
>
> ⚠️ **Snímek v čase — ověř proti HEAD (CH-081).** Verdikty níže jsou stav z běhu, ne stav kódu.
...
| undo | `POST .../accounts/:id/undo` | `undoLast` | `withTransaction` (read+write v jedné tx, retry na write-conflict) ✅ |
| transfer | `POST .../accounts/:id/transfer` | `transfer` | `withTransaction` + atomické append ✅ |
| nákup | `POST /api/campaign/shopitems/:id/purchase` | `CampaignPurchaseService.purchase` | `withTransaction` + atomický `$gte` floor + `clientNonce` idempotence ✅ |
| storno | `POST /api/campaign/purchases/:id/refund` | `refund` | `withTransaction` (atomický flip `active→refunded` + kredit) ✅ |

## Seed kandidáti (hypotézy → verdikt z běhu)

| ID | Třída | Uzel | Hypotéza | Repro | Verdikt |
|---|---|---|---|---|---|
| **K-RC-E1** | TOCTOU | [campaign-purchase.service.ts:144](../../../Projekt-ikaros/backend/src/modules/campaign/services/campaign-purchase.service.ts#L144) | `account.balance < paidAmount` čteno PŘED `$inc` bez floor → 2 nákupy → záporný zůstatek | `Barrier(2)` na `adjust` | ✅ opraveno (`debitIfSufficient` $gte floor) |
| **K-RC-E2** | DP | [campaign-purchase.service.ts:228](../../../Projekt-ikaros/backend/src/modules/campaign/services/campaign-purchase.service.ts#L228) | `status!=='active'` check není atomický → 2 storna → peníze 2× | `Barrier(2)` na `adjust` | ✅ opraveno (`markRefundedIfActive` atomický flip) |
| **K-RC-E3** | LU | [character-accounts.service.ts:410](../../../Projekt-ikaros/backend/src/modules/character-subdocs/character-accounts.service.ts#L410) | `undoLast` čte tx pole → `$set` slice; adjust mezi read↔write → vklad zmizí | `Gate` na `repo.update` | ✅ opraveno (`withTransaction` + retry, [test :233](../../../Projekt-ikaros/backend/test/race/economy.race.e2e-spec.ts#L233)) |
| **K-RC-E4** | LU | [campaign-purchase.service.ts:315](../../../Projekt-ikaros/backend/src/modules/campaign/services/campaign-purchase.service.ts#L315) | `addToInventory` full `sections` `$set` → 2 nákupy téže postavě → položka zmizí | `Barrier(2)` (sdílí scénář E1) | ✅ opraveno (`appendItemToSection` $push) |
| **K-RC-E5** | AT | campaign-purchase.service.ts (celé) | nákup přes 3 moduly bez transakce → pád na purchase-logu → položka+peníze pryč, bez storno-záznamu | `withGate` + throw na purchaseRepo | ✅ opraveno (`withTransaction` + FA test :317) |
