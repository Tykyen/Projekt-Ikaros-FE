...
**🚧 Implementováno (kód hotový, 2026-06-19, `spec-14.3`) — ČEKÁ report-only deploy → enforce:** Klíčová oprava zadání: **hlavní XSS-CSP patří na FE nginx** (servíruje HTML dokument), ne na BE (vrací jen JSON/obrázky). Rozděleno: **FE [nginx.conf→default.conf.template](../../default.conf.template)** = jádro (CSP, HSTS, Referrer-Policy, Permissions-Policy; per-env `BACKEND_HOST` přes nginx envsubst; inline pre-hydration skript přes **SHA-256 hash** + build guard [check-csp-hash.mjs](../../scripts/check-csp-hash.mjs)). **BE [main.ts](../../../Projekt-ikaros/backend/src/main.ts)** = `helmet()` hardening (API `default-src 'none'`, HSTS, nosniff, `frame-ancestors 'none'`, `crossOriginResourcePolicy:false` kvůli PixiJS `/static/` texturám). CSP zavedena jako **report-only** (`CSP_HEADER_NAME` přepínač) → po sběru porušení na produkci přepnout GitHub var na enforce. **ZBÝVÁ:** (1) deploy report-only + BE restart, (2) ruční ověření `/static/` WebGL textur, (3) sběr CSP violations (pozor TipTap/PixiJS/dice WASM mohou chtít `style-src 'unsafe-inline'` / `script-src 'wasm-unsafe-eval'`), (4) přepnout na enforce. Follow-up **14.3-a**: serverový `report-uri` sběr.
