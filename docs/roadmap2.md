...
- **Dračí Doupě II** (`drd2`) — *stav kódu:* list sheet+testy, mapa combat panel `Drd2CombatPanel`
  ...
  - [x] **Bestie (grafika)** — ✅ 2026-06-28 `Drd2BestiePanel` + `Drd2BestieCombatActions` (spec-16.2e; fantasy pergamen statblok). **Sudba = HP (damageable)** + `−/+`; Hranice T/D/V reference (nehází se, edit velikostí); **Charakteristiky = klik 2k6+úroveň**; ZS = poznámky; inline edit. Schéma rozšířeno: `drd2/bestie.json` (Sudba damageable + charakteristiky/ZS list) + `drd2/token.json` (zrcadlí pro edit, sanitizace proti BE strict) → bestiář editor (generic EntitySchemaForm) + spawn HP automaticky. export-schemas do BE (čeká BE restart). Ověřeno build+6 testů+eslint; vizuál po deployi.
...
- 🦄 **MLP / pastel** (id `anime`, label „MLP") — *8. styl, přidán v 16.2c (Matrix).* Měkké pastely (levandule/mint/broskev/baby-blue), zaoblené „nálepkové" karty, jemné duhové gradienty; signature **cutie-mark emblém + obláčkový rám**. Roztomilé, ale vkusné a čitelné (ne dětské čmáranice).
...
### - [x] 21.3 Stavitel & generátor podzemí / map — [H4-01 · dopad střední · náklad velký] 🔁 ✅ *(2026-07-14, a–g; perzistence krajiny opravena 2026-07-20, D-077)*
...
**Otevřené otázky:** ~~Jen dungeony, nebo i exteriéry/města? Procedurální layout vs. jen knihovna dílků?~~ (vyřešeno spec 21.3: základ = jen dungeony, procedurální rooms-and-mazes; exteriéry/dílky = výhled §9)
- **21.3a Editor + generátor ✅ FE+BE (2026-07-14)** — per-world route **`/svet/:slug/podzemi`** (nahradila admin stub `/admin/dungeon-builder`): fullscreen donjon editor (canvas 2D, nástroje podlaha/guma/6 typů dveří dle legendy/schody/terén/**14 dekorací** (bedna·postel·stůl·židle…)/popisky, undo/redo 50, zoom/pan+pinch) + **deterministický generátor** (rooms-and-mazes, hustota/křivolakost/zvláštní dveře/seed, „Přegenerovat") + seznam s miniaturami + PNG export (pergamenový rám + legenda). **Gating 19.4:** tvorba = Hrac+ ∧ (Podporovatel ∨ PJ+), hráč vidí jen svoje, PJ vše; BE `dungeon-maps` rozšířen (ownerId, 4 typy dveří, label dekorace, DTO limity 100×100, NOT_SUPPORTER/NOT_DUNGEON_OWNER). Engine 11 vitest + BE 32 jest testů. Spec: [arch/phase-21/spec-21.3-tvorba-podzemi.md](arch/phase-21/spec-21.3-tvorba-podzemi.md).
- **21.3c Knihovna + přenos mezi světy ✅ FE+BE (2026-07-14)** — `worldId` volitelné (bez něj = **osobní knihovna**, vzor MapTemplate); `POST /dungeon-maps/:id/copy` (kopie do knihovny ∨ do světa — VŽDY kopie, tenant izolace) + `GET /dungeon-maps/library`; taby „V tomto světě | Moje knihovna" + „Kopírovat do světa…" (výběr z mých světů) + platformová stránka **`/ikaros/podzemi`** s editorem v library režimu. Gating: do knihovny Podporovatel ∨ PJ+ zdroje (`NOT_LIBRARY_ELIGIBLE`), do světa create brána cíle; library položky owner-only (ani admin). BE 49 jest testů.
- **21.3d Hloubka obsahu ✅ FE (2026-07-14)** — katalog dekorací **14 → 47** v 6 kategoriích (nábytek/kontejnery/dungeon/jeskyně/tábor/markery pro PJ, vč. trůnu, kostry, magického kruhu, stalagmitů, pavučin…); **povrchy podlahy** (dlažba/dřevo/hlína/písek/tráva — nástroj tažením, `floorVariant`); **auto-zabydlení generátoru** (slider Zabydlenost: typy místností dle velikosti + šablony nábytku, deterministické, dveře nikdy neblokuje; 6 testů).
- **21.3b Export na taktickou mapu ✅ FE+BE (2026-07-14)** — tlačítko „Na taktickou mapu" (PJ, world režim): render 1:1 → upload → `export-scene`; BE nově generuje **`MapWall[]` zdi (run-merge hran podlaha↔skála) + dveře** (zavřené; zamčené/mříž locked; průchod bez zdi) + `gridType:'square'` → scéna připravená na dynamické vidění/LoS. 7 unit testů konverze.
- **21.3e Město ✅ FE+BE (2026-07-14)** — nástroj přejmenován na **„Stavitel"**: při založení volba 🕳️ Podzemí / 🏘️ **Město** (`mapKind`, nekonvertuje se, replace/copy ho zachovávají). Město = pozitiv na terén: buňky ulice·budova·hradba·brána·most, nástroje per druh, 6 nových dekorací (stánek, vozík, lucerna, strom, keř, plot). **Generátor města**: řeka s mosty → hlavní ulice + uličky rekurzivním dělením → náměstí s kašnou/stánky → parcely budov → hradby s bránami a věžemi → zeleň → číslování budov + **garanční prune souvislosti ulic**; parametry hustota zástavby/křivolakost/hradby/řeka/zeleň/zabydlenost/seed. Export na TM: budovy+hradby blokují výhled, brány = dveře (kind-aware walls util). Testy: 10 city engine vitest + BE 54 jest.
- **21.3f Hloubka podzemí ✅ FE+BE (2026-07-14)** — **Klíč mapy** (`notes` na dokumentu: popisek → titulek+text pro PJ; panel v editoru s ikonou knihy; tiskne se pod PNG legendu; kopie ho přenáší; funguje pro všechny druhy map) · **témata generátoru** (klasika/hrobka/doly/kanály/pevnost — tematické místnosti, nábytek, povrchy podlah i distribuce dveří: hrobka bez mříží + tajné, pevnost bez tajných + zámky/mříže, doly hliněné štoly) · **organické jeskyně** (cellular automata, největší komponenta + tunely k bublinám, jezírka, stalagmity; propojenost testem).
- **21.3g Krajina ✅ FE+BE (2026-07-14; perzistence opravena 2026-07-20)** — ⚠️ **Historie:** `mapKind: 'wilderness'` se v `toEntity` kolabovalo na `'dungeon'` a `replace()` (overwrite) to zapisovalo zpět → krajina se po prvním uložení tiše měnila na podzemí; větev `wilderness` v `dungeon-walls.util.ts` byla pro reálná data mrtvá, takže ani LoS export nefungoval. Odhaleno adversariální verifikací v R3 24.5 (**D-077**, uzavřen): sdílené výčty + `pickEnum()` v `dungeon-map.interface.ts` = jeden zdroj pro DTO · schema · `toEntity`, `enum` ve schématu, FE `normalizeMapKind()`; regresní test **přes repozitář** (`dungeon-maps.repository.spec.ts`, ověřen červeno-zeleně). Vše ostatní z 21.3g hotové — třetí druh mapy 🌲 (plný generátor exteriérů): deterministický **value-noise (fBm)** → hory/kopce/les/mokřad/louka dle elevace+vlhkosti, řeka po spádu + jezero, **cesta greedy po nejnižším terénu** (přes vodu mosty, v každém sloupci mapy), volitelná **vesnička** (očíslovaná stavení + pole okolo), zeleň/tábor dle zabydlenosti. Buňky les·hory·kopce·pole·mokřad (+ reuse cesta/voda/most/budova), nástroje s canvas náhledy, legenda per druh. TM export: les+hory+budovy blokují výhled. Testy: 12 vitest (f+g) + BE 55 jest.
