...
| **Character directory** (adapter nad `/pages/directory`; queryKey zůstal `characters` kvůli C-15 invalidaci) | `['characters', worldId, 'directory']` (factory) | WorldLayout sidebar (slot postavy člena), MyCharacterPage, GroupMembersPage, MembersTab, mapa MapEmptyState (spawn z postav), TransferModal (cíl), SettingsAccountSection | 30s; `!!worldId` | [useCharacterDirectory.ts:15](../../src/features/world/pages/api/useCharacterDirectory.ts#L15) |
