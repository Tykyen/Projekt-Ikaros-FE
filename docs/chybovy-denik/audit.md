...
## ✅ ŘEŠENÍ — revize dluhů proti kódu + dávka 12 oprav (D-064/065/070/071/072/066a + SEC/LAUNCH-GAP položky) — 2026-07-12

**Kontext.** Uživatel: „pročistit dluhy, smazat opravené, opravit co jde". Postup: 5 paralelních read-only agentů ověřilo KAŽDOU položku dluhy.md proti HEAD (FE+BE) + plný vitest běh → teprve pak úpravy souboru a odsouhlasená dávka oprav (FE → BE → herbář, nemíchané).

**Co zabralo — revize.** Kódem ověřená revize našla ~15 už opravených položek (SSRF, Redis, JWT+tokenVersion, TOTP lockout, HP clamp, XSS title, writeConcern, Tyky bypass, Meili warn, role práh map, GDPR export FE, push spam…) → smazány s odkazem na git log; částečně opravené ZÚŽENY na reálný zbytek (erasure = vědomý právní design, ne zapomenutý kód). **D-064 překvapení: runner dávno jede** (462/467 zelených) — dluh popisoval stav z minulosti; 14 failů mělo 5 kořenů, hlavní = testy nevyplňovaly nový POVINNÝ `ageBracket` (20C) → zod refine (shoda hesel) se bez validní base VŮBEC nevyhodnotí a submit nikdy neproběhl (vysvětlilo všech 7 RegisterModal failů najednou).

**Co zabralo — opravy (vše testy+typecheck+lint zelené).** FE: 14 testů (ResizeObserver stub do setup, mock GrowthSection/CostsSection, ageBracket, WorldCard anon+private=„Soukromý svět" je záměr, StoryMapPill label „Mapa") → **suita 467/467, 3547 testů**; „Odhlásit se ze všech zařízení" (karta Aktivní relace, `useLogoutAll` + sdílený `clearLocalSession`) + `SESSION_REVOKED` instant-logout v client.ts. BE: undo guard (PT-43 — `origin:'purchase'`/`transferRef` → 409 + gate `assertCanAdjust`), scheduled-messages atomický claim (`findOneAndUpdate` pending→sending + stale-recovery 10 min), unban dle zdroje (`bannedBy:'moderation:<decisionId>'` + legacy fallback přes banReason; 5 nových testů), login timing-equalizer (dummy bcrypt hash z uuid při startu). Herbář: `plant` do ReportTargetType (BE+FE) + enforcement listener (vzor bestiae; M4=hard delete, revert nevratný→warn) + ReportButton na detail rostliny I bestie (D-066a), `media.orphaned` při delete/výměně obrázku, `rarity:null` sentinel → `$unset` (DTO přes `OmitType`, jinak TS2416).

**Zhodnocení.** DOBŘE: revize PŘED mazáním (dluhy.md tvrdil věci ~3 týdny staré — bez ověření bych smazal špatné a nechal mrtvé); ageBracket kořen našel jeden Read schématu místo ladění 7 testů zvlášť; batch držel FE/BE oddělené. POUČENÍ: dluhy stárnou rychle — u dlouhoběžícího dluhu (D-064) nejdřív reprodukuj, pak čti popis; `@IsOptional` v class-validator pouští i `null` (hodí se pro clear sentinely); override zděděného pole DTO s širším typem = `OmitType` + re-deklarace. ZBÝVÁ (domluveno): dicePayload server-recompute + refund plná tx (vlastní specy), route-audit CI (PAT od uživatele).

## ✅ ŘEŠENÍ — D-AUDIT-2026-07-11 rozpuštěn: kontejner zbytků auditu ověřen proti HEAD a triažován — 2026-07-19

**Kontext.** Uživatel: „vyřeš D-AUDIT-2026-07-11, až vyřešíš vymaž". Šlo o poslední sběrný kontejner zbytků plného auditu (46 stylů) — žádná 🔴, jen 🟠 strukturální + ⚪ self-refuted + blokováno-na-uživateli + neproběhlé proof-vrstvy. Kontejner „nešel řešit jako celek" — stejná třída jako dnes rozpuštěné D-SEC-GAP / D-LAUNCH-GAP.

**Co zabralo — NE kód, ale triage.** Klíčové rozhodnutí: **žádný z nálezů není bezpečný tichý code-fix** (presence room-scoping = produktové rozhodnutí kdo koho vidí online; `autoIndex` off bez `syncIndexes` deploy kroku = tiše horší než dnešek; RC-E8 lock = strukturální). Base rule zakazuje strukturální změny bez souhlasu → „vyřešit" = to samé co u předchozích kontejnerů: ověřit proti HEAD, mrtvé/vyvrácené smazat, živé strukturální přesunout do **Odložené** se správným triggerem, kontejner smazat.

**Jak ověřeno.** 3 paralelní read-only verifikační agenti (realtime/scale · RC-E8+DUN-1 · PERF+db-integrity+AR-META), každý vrátil verdikt PLATÍ/HOTOVO + evidence soubor:řádek. Výsledek: **PLATÍ** (živé) — presence globální broadcast (`presence.gateway.ts:107/153/157`), žádný connection cap, plain emit, RC-E8 (`character-inventory.repository.ts:50` `$set` celé sekce), DUN-1 (alias↔kanonický klíč soft-skip `map-operations.service.ts:1752`), autoIndex ON (`database.module.ts` bez klíče), AR-META-1 (267 nálezů `guards:[]`, F-20=G0). **HOTOVO** (smazáno) — `compression` (v BE kódu není, řeší Caddy), db-integrity kořen (dokument sladěn na 46 kolekcí, zdroj pravdy = `world-hard-delete.service.ts`). **⚪ self-refuted** (smazáno) — N-TM-01 listener leak, RC-P3 slug souběh.

**Kam přesunuto (Odložené).** `D-RC-E8` (inventář lost-update, sesterský RC-E7), `D-RT-SCALE` (presence broadcast + cap + volatile; Redis-presence sledován v D-NEW-chat-presence-scale, neduplikováno), `D-PERF-BE` (autoIndex+syncIndexes deploy krok, N+1, seznamy bez limitu), `D-DUN-1` (alias systemStats validace), `D-AUDIT-HARDENING` (AR-META guardy + db-integrity rerun + proof-vrstvy + SLO), `D-DB-BACKUP-CRON` (blokováno na uživateli).

**Zhodnocení.** DOBŘE: opět potvrzeno, že **report ≠ stav** — každý nález znovu ověřen proti kódu (2 ze ~15 už byly hotové, jinak by se relokovaly mrtvé dluhy); paralelní verifikace s evidencí soubor:řádek = žádné hádání; triage místo tichého kódování respektuje base rule (strukturální → diskuze). POUČENÍ: „vyřešit kontejner dluhů" často NEznamená napsat kód — znamená rozštěpit na atomické dluhy s triggery + smazat obal; hodnota = příště se stejné nálezy neověřují znovu a neotevře se vyvrácené (N-TM-01/RC-P3). ZBÝVÁ: nic — kontejner smazán, Otevřené = jen D-063.

## ✅ ŘEŠENÍ — návazně: 5 relokovaných audit-nálezů reálně opraveno kódem (BE) — 2026-07-19

**Kontext.** Po triage výše uživatel: „myslel jsem že je vyřešíš, ne rozebereš … vyřeš vše co můžeš". Korekce: triage byla půl-práce — část relokovaných nálezů šla opravit kódem bez produktového rozhodnutí. Rozdělil jsem na řešitelné-teď vs. blokované (produktové/mrtvý-kód/živý-stack/uživatel) a to řešitelné zavřel.

**Co zabralo (5 fixů, vše BE, testy+typecheck+lint zelené).**
- **RC-E8** inventář lost-update → atomický `$pull` `removeItemFromSection` (repo) + `removeInventoryItem` (service) + přepnutý `removeFromInventory` caller. **Symetrie k RC-E4 append** (stejná cesta `sections.$[sec].items` + arrayFilters, native driver kvůli Mixed schématu, jen `$pull` místo `$push`) — čistší než optimistic lock, žádný 409/caller change. Refund testy přepnuté na `removeInventoryItem`.
- **DUN-1** alias systemStats validace → BE port `resolveSystemId`+`SYSTEM_ALIASES` (`common/rpg/system-id.ts`, dual-source zrcadlo FE) + normalizace v **`SchemaRegistryService.get()`** = JEDNO lookup hrdlo → opravilo token I bestie najednou; sladí BE výběr schématu s FE engine resolucí. + unit spec (parita + fixed-point aliasů).
- **autoIndex** → `autoIndex:false` v prod + `DatabaseIndexSyncService` (`OnApplicationBootstrap`) běží `syncIndexes()` **na pozadí** (neblokuje boot, escape hatch `DB_SYNC_INDEXES=0`); 88 schema-level indexů → sync bezpečný.
- **volatile.emit** → `.volatile` na typing/ping/ruler (vysokofrekvenční, drop-safe). **Presence záměrně vynechána** (nízkofrekvenční stav; `offline` frame zahozený saturovanému klientovi = trvale stále-online; volatile je proti floodu, presence nefloodí). maps.gateway.spec mock rozšířen o `.volatile`.
- **AR-META F-20/R-02** → F-20 canary test (spadne, když se top-level `abilities` vrátí jako validované pole) + R-02 anotace krycího testu + cílené manuální guardy v `anti-regression-map.json`. Scanner: **G0→G3** (G3 146, fixed&G0 12→2).

**Jak ověřeno.** `tsc --noEmit` čistý; eslint čistý; jest 275 zelených (campaign-purchase 68 vč. přepsaných refund testů, create-bestie+characters+maps.gateway, system-id 6, chat/presence/character-subdocs/bestiae 201); `anti-regression-scan.mjs` exit 0, F-20/R-02 zmizely z fixed&G0.

**Zhodnocení.** DOBŘE: kořenové hrdlo (registry `get()`) opravilo DUN-1 pro token i bestie jednou změnou místo N call-sitů; RC-E8 řešen symetrií k existujícímu doloženému vzoru (RC-E4), ne vymýšlením; každý fix ověřen před uzavřením dluhu, ne naslepo. POUČENÍ: **před rozebráním dluhu na „Odložené" nejdřív roztřídit řešitelné-teď vs. skutečně-blokované** — relokovat řešitelný nález = zbytečně odložená práce (uživatelova korekce byla oprávněná). ŠPATNĚ: první reflex byl „triage a hotovo", ač 5 z položek šlo rovnou opravit.

**Dodatek (uživatel: „D-PERF-BE a D-AUDIT-HARDENING mi připadají vyřešené").** Ověřil jsem zbytky do hloubky místo domněnky: **notifyUsers N+1** = reálný (`notify(id)`→`findByUserId` per příjemce) → opraveno batch `findByUserIds` (`$in`, 1 dotaz; `filterByCategory` už batchnutý byl) + přepsané testy (28/28). **getUnreadCounts** = 2 count/kanál, ale N=kanály/svět (malé, indexované, nezávislé na počtu světů) → **bounded, ne škálovací riziko** → z dluhu ven. **GET /worlds/pages** = reálně bez limitu (`findAll` vrací vše) → **breaking pro FE** → zůstává jako jediný D-PERF-BE bod. **D-AUDIT-HARDENING smazán z dluhy.md** — db-integrity rerun + proof-vrstvy + SLO NEJSOU code-dluhy, jsou to verifikační/audit kroky vyžadující živý stack a **už sledované v BE audit-plánech** (ne-code-dluh do dluhy.md nepatří). POUČENÍ: „připadá vyřešené" ověř do hloubky — část byla reálný N+1 (opravit), část bounded (smazat), část ne-code-dluh (reklasifikovat), ne všechno stejně. ZBÝVÁ (Odložené, oprávněně): presence room-scoping (produktové), connection cap + Redis presence (mrtvý kód na 1 instanci), GET /worlds paginace (breaking FE), DB zálohy (uživatel); db-integrity/proof-vrstvy/SLO žijí v audit-plánech.
