...
### ✅ ŘEŠENÍ — SCA brána spadla na react-router advisory bez vydaného fixu (RSC mode, který nepoužíváme) → dokumentovaný allowlist s expirací, ne downgrade/vypnutí brány · 2026-07-24
**Kontext:** CI job „Frontend build, lint & nav audit" spadl na kroku SCA (`npm audit --omit=dev --audit-level=high`) — čerstvé advisory GHSA-qwww-vcr4-c8h2 „React Router: RSC Mode CSRF Bypass" na react-router 7.18.0 (2 high). Nesouviselo s commitem 27.3, jen CI běžel na něm.
**Diagnóza:** advisory je v **RSC / Framework módu** react-routeru (server actions). Projekt je Vite SPA s `createBrowserRouter` — grep `RSC/ServerRouter/framework/@react-router/dev` = **0** → zranitelná cesta u nás neexistuje (false-positive dle architektury). **Fix ale není kam:** `latest = 7.18.1` (pořád ve vulnerable range `>=7.12.0 <8.3.0`), v8 nevyšla; jediný `fixAvailable` = **downgrade na 7.11.0** (`isSemVerMajor`, breaking, ztráta features 7.12–7.18). `npm audit` hlásí podle VERZE, ne podle použití.
**Co zabralo:** guard `scripts/check-sca.mjs` (vzor `check-bundle-budget.mjs`) místo holého auditu: spustí `npm audit --json`, projde `vulnerabilities[].via` (objekt = přímé advisory, string = tranzitivní přes jiný balík → řeší se u zdroje), a propustí JEN advisory na **ALLOWLIST** (match GHSA z `via.url` NEBO číselný `source`), každé s `reason` + `reviewBy`. Na jakékoli jiné high/critical selže stejně jako dřív. **Expirace:** po `reviewBy` (2026-09-30) guard spadne → nutí revizi (výjimka není věčná). Datum přes env `SCA_TODAY` (CI `date -u +%F`), ne `new Date()` (deterministický log). CI krok = `npm run audit:sca`.
**Jak ověřeno:** 3 scénáře lokálně — (1) dnes → exit 0 (propustí s vysvětlením), (2) `SCA_TODAY=2026-12-01` → exit 1 (expirace), (3) rozbitý match v dočasné kopii → exit 1 „neomluvené high" (brána drží na vše ostatní).
**Zhodnocení (dobře/špatně):** **dobře** — nešel jsem cestou nejmenšího odporu (downgrade/`--audit-level=critical` by oslabil bránu i pro budoucí *jiné* nálezy); allowlist je úzký (1 GHSA), dokumentovaný, s expirací a fail-safe (chybějící/nevalidní JSON → fail). **Správně prodiskutováno předem** (bezpečnostní risk-accept = uživatelovo rozhodnutí, ne moje). **Pozor dál:** allowlist je skutečné přijetí rizika — platí JEN dokud grep potvrzuje, že RSC mode nepoužíváme; kdyby projekt přešel na framework/RSC mode react-routeru, výjimku OKAMŽITĚ zrušit. Až vyjde patch (7.x nebo v8 ≥ 8.3.0) → Dependabot bumpne, `stale` hláška guardu připomene úklid allowlistu.
**Příznak (anti-pattern):** SCA brána blokuje deploy na advisory bez fixu → sáhnul bych po downgrade/vypnutí prahu místo ověření, jestli se cesta vůbec týká architektury, a úzké dokumentované výjimky s expirací.
...
**Zhodnocení (dobře/špatně):** **dobře** (architektura) — čistý průchod bez cyklení; klíčové rozhodnutí *render čte registr, ne mění data* ušetřilo prop-drilling a udrželo definice položek nedotčené; **scoped** volba (nic hotového neschováno, C jen už-blokované) předešla regresi pro zavedené PJ; SSOT set + render přes stabilní klíč je přenositelná metoda. **ŠPATNĚ (vizuál) — vlastní chyba:** u samotného štítku jsem si řekl „na tak malý prvek frontend-design přeháním" a udělal `<PreviewBadge>` narychlo (⚗ + warning barva). Vlastník to po nasazení zamítl („vzhledově hrozné") → **badge SUNDÁN** (komponenta smazána, 7 render míst, changelog zrušen; registr/klasifikace/freeze pravidlo zůstalo). **Lekce:** i drobný vizuální prvek, který jde do produkčního UI napříč všemi skiny, zaslouží `frontend-design` návrh + odsouhlasení PŘED nasazením — „je to malé" není důvod přeskočit design (a `mobil-desktop` ověří jen že se to vejde, ne že to vypadá dobře). **Příznak:** přeskakuju design u „malého" UI prvku → nasadím → uživatel to zamítne kvůli vzhledu → revert.
...
**Proč to je správně:** diary-backed (PC i NPC přes `characterSlug`) → **0 BE změn**; HP/zdroj fill přes `var(--dd-hpfill-grad,…)` (ne inline hardcode, past §9.15); system-id alias `draci-hlidka→drdh` řeší konzument přes `useResolvedSystemId` (netřeba měnit).
...
**Kontext:** 16.2d DrD+ grafický průchod — udělal jsem deník, combat panel i bestii, ale obal tokenu + samostatné mapové/chat embedy zůstaly na tmavém mapovém `--map-ui-*`, nesladěné s pergamenem. Uživatel narazil ("hody, dicelog i to kolo bestie není stejné jako deník … není to poprvé").
**Co jsem udělal špatně:** považoval jsem systém za "hotový" po deníku+combat+bestii a NEpropsal embed skin (sada `--dd-embed-*` + větve `[data-diary-system='drdplus']` ve 4 modulech). U drd16 jsem to udělal, u drdplus zapomněl — opakovaný vzor (předtím sci-fi/embed).
...
**Jak ověřeno:** `resolveCharacterHp.test.ts` 29/29 (`--pool=threads`; default forks pool tady timeoutuje — viz ✅ ŘEŠENÍ „PC HP bar na mapě (JaD+dnd5e)" · 2026-06-29) + `npm run build` ✓. Živě po deployi (FE pushuje uživatel).
...
### CH-128 — vitest z Git Bash kontextu padá i přes `npm run test:run` (doplněk CH-029); funguje jen PowerShell · 2026-07-12
...
### ✅ ŘEŠENÍ — 21.2a V10: filtr kategorií sad + přízviska s rodovou shodou bez změny schématu · 2026-07-14
**Co zabralo:** (1) **Filtr typu sad** v záložce Jména = chips Vše/Fantasy (rasy)/Státy světa/Vlastní nad selectem — kategorie `NameSetCategory` už existovala, jen zúžení `visibleSets` + zrušení výběru při nekompatibilním filtru; label `morvol` přejmenován na „Fantasy (rasy)" (řeč uživatele). (2) **Přízviska**: engine i checkbox V7 existovaly, ale všech 76 seedovaných sad mělo `epithets: []` → checkbox trvale šedý. Klíčové rozhodnutí = **rodová shoda BEZ rozšíření schématu**: datová konvence povoluje jen dva tvary přízviska — jednoslovné adjektivum na „-ý" (strojově -ý→-á pro ženy, `feminizeEpithetCs`) nebo rodově neutrální předložkovou frázi („z Mlžných hor") — víceslovná adjektiva zakázána (nešla by přechýlit: „Bílý vlk"→„Bílá vlk"). Obsah generuje agent s tvarovými pravidly vynucenými regexem; inject skript pravidla validuje znovu před zápisem do seedu.
...
### ✅ ŘEŠENÍ — 21.3c+d+b Podzemí: knihovna přes kopie, zabydlení sdíleným rng, zdi na TM run-merge hran · 2026-07-14
**Co nakonec zabralo:** (c) Osobní knihovna = dokument s `worldId: null` (vzor MapTemplate) + `POST :id/copy` — VŠECHNY přenosy mezi světy jsou kopie, nikdy reference (tenant izolace se neřeší, protože žádná hranice nevzniká); „PJ někde" pro library gate se odvozuje z role ve ZDROJOVÉM světě → nula nových membership dotazů. (d) Auto-zabydlení jede na témž rng jako generátor → stejný seed = stejný nábytek zadarmo; glyfy (33 ks) psal paralelní agent podle přesného stylového kontraktu (lw/inset/PAPER_COLORS) a prošly na první tsc. (b) Zdi pro LoS = run-merge hran podlaha↔skála (L mapa ~stovky segmentů místo tisíců), dveře = kolmé `door` segmenty; obraz scény render 1:1 `cellPx = cellSize` → mřížka TM sedí bez kalibrace.
**Proč správně:** sdílení referencí přes světy by vyžadovalo cross-world read pravidla (protrhlo by leak-safe model); kopie je blbuvzdorná. Editor v library režimu = TATÁŽ komponenta, režim z absence `worldSlug` param — žádná druhá stránka. Controller: statická route `library` deklarovaná PŘED `:id` (jinak ji spolkne param route).
**Jak ověřeno:** BE 49 jest (matice copy/library/owner-only vč. „admin do cizí knihovny nesmí" + 7 testů konverze zdí na ASCII gridech), FE tsc+eslint 0, engine 17 vitest (determinismus zabydlení, dveře neblokované, limit 500), HelpPage 25. Plný build+vitest doběhl souběžně.
**Zhodnocení:** dobře — 3 sub-kroky v jednom zátahu bez cyklení díky spec-first; agent na mechanické glyfy = správná dělba (kontrakt v promptu > review ex post).

---

### ✅ ŘEŠENÍ — 21.3e Město: druhý generátor stavitele jako `mapKind` na téže entitě · 2026-07-14
**Co nakonec zabralo:** město NENÍ nová entita/nástroj, ale `mapKind: 'dungeon'|'city'` na DungeonMap — knihovna, kopie, PNG, export na TM a gating fungují pro města ZDARMA. Sémantika je zrcadlová: podzemí = negativ (kreslíš chodby do skály), město = pozitiv (stavíš budovy na terén) → tentýž buňkový model, jen se přepne význam `empty`, paleta nástrojů a engine. Generátor města: řeka → hlavní kříž ulic → rekurzivní dělení bloků → náměstí → parcely u ulic → hradby s bránami → zeleň/zabydlení; souvislost ulic jistí **garanční prune** (flood fill od hlavní křižovatky, odříznuté pahýly zpět na terén) — první běh testů chytil pahýly uříznuté řekou (3 komponenty místo 1).
**Proč správně:** samostatná entita by zdvojila list/knihovnu/copy/export a „přepínání generátorů" (přání uživatele) by neexistovalo. ⚠️ Dvě overwrite pasti pohlídány testem: `replace` i `copy` musí `mapKind` explicitně přenést (jinak první uložení města udělá dungeon). Kind-aware walls util = jen jiná množina „blokuje pohled" + jiná množina „dveří" — jedna funkce, ne dvě.
**Jak ověřeno:** BE 54 jest (vč. city walls na ASCII gridech: budova=4 obvodové zdi, brána=door, otevřený okraj mapy bez zdí), city engine 10 vitest (determinismus, souvislost ulic napříč hradby×řeka, hustota, limit dekorací), FE tsc+eslint 0, build ✓, HelpPage+builder 52 testů. Glyfy města opět paralelní agent dle kontraktu (druhé použití vzoru, opět čisté napoprvé).
**Zhodnocení:** dobře; eslint past — komentář mezi prázdnými `case` větvemi spouští no-fallthrough (přesun na řádek case). Celý 21.3 (a–e) hotový za jeden den díky spec-first a sdílené infrastruktuře.

---

### ✅ ŘEŠENÍ — 21.3f+g Stavitel: klíč mapy, témata+CA jeskyně, krajina přes value-noise · 2026-07-14
**Co nakonec zabralo:** (f) Klíč mapy = `notes` klíčované TEXTEM popisku (ne id dekorace) — přežije přegenerování se stejnými čísly, funguje pro všechny druhy map a PNG ho tiskne pod legendu (výška rámu se dopočítává z zalomených řádků PŘED kreslením). Témata podzemí = jen datové přepínače nad existující pipeline (pooly místností, váhy dveří, povrchy) — žádný nový algoritmus; organické jeskyně = samostatný CA generátor (largest-component + tunely k bublinám). (g) Krajina = třetí `mapKind`; klasifikace terénu z deterministického value-noise fBm (vlastní hash, žádné Math.random), cesta = greedy walk po nejnižší elevaci (hora není zákaz, jen drahá) → přirozeně obchází masivy a vždy protne mapu (testem po sloupcích).
...
### ✅ ŘEŠENÍ — 23.5 sliding session: kořen „odhlásí mě do 3 dnů" = race souběžných refreshů (FE single-flight + BE grace okno 60 s) · 2026-07-19
**Kontext:** Uživatel: „token se opravdu restartne po méně než 3 dnech, zkontroluj hloubkově." Karta 23.5 tvrdila zastaralou cross-site hypotézu — ta byla vyvrácená už 2026-06-24 (viz ✅ ŘEŠENÍ tam: same-origin přes Caddy, refresh TTL 3→60 d). Přesto odhlašování pokračovalo.
**Diagnóza (deep-dive, čteno FE+BE+deploy):** (1) prod konfigurace SPRÁVNĚ — GitHub vars `JWT_EXPIRES_IN`/`JWT_REFRESH_TTL_DAYS` nenastavené → compose defaulty `1d`/`60` (`${VAR:-default}` pokrývá i prázdnou hodnotu z deploy `.env`). (2) Skutečný kořen = **race souběžných refreshů**: FE interceptor spouštěl VLASTNÍ `POST /auth/refresh` pro každý 401 (žádný single-flight) — načtení stránky po expiraci access tokenu (1 d) vystřelí N paralelních dotazů → N×401 → N refreshů se STEJNOU cookie. BE rotace bez grace: první jti zrotuje, druhý → reuse-detection → `REFRESH_TOKEN_ABUSED` → **revoke celé rodiny** → toast „Přihlášení vypršelo" + logout. Race byla v kódu PŘIZNANÁ komentářem („Akceptujeme"). Sliding session tak nikdy nepřežila první expiraci access tokenu.
**Co zabralo — fix (spec-23.5):** ① FE `client.ts` — single-flight `refreshAccessToken()`: modulový sdílený promise, souběžné 401 čekají na jeden request; toast+logout při failu právě 1× (uvnitř sdíleného flow), čekající jen reject. ② BE `auth.service.ts` — grace okno 60 s: při rotaci se nástupnický pár uloží in-memory pod STARÝM jti (`rememberGracePair`, lazy úklid); reuse v okně vrátí TÝŽ pár (řeší multi-tab, PWA vs. prohlížeč, síťový retry — FE single-flight je nepokryje); reuse PO okně = reuse-detection jako dřív (detekce krádeže zpožděna max 60 s = standardní „reuse interval", Auth0 vzor). Cache in-memory (1 replika BE); při scale-out přesun do Redisu (vzor `THROTTLER_REDIS`).
**Jak ověřeno:** BE typecheck + lint:check + prettier ✓, jest `auth.service.spec` **80/80** (2 nové: reuse v okně → týž pár + rodina žije + rotace 1×; reuse po okně → 401 + revoke rodiny přes `Date.now` spy). FE `npm run build` (tsc -b + vite) ✓ + vitest **17/17** (nový `refreshAccessToken.spec` 3/3: 3 souběžná volání = 1 request; reset flightu; fail = 1 toast+redirect). Živě po deployi: po >1 dni Network tab → právě 1 refresh → 200, žádný ABUSED.
**Zhodnocení:** **dobře** — deep-dive šel od symptomu k datům (GitHub vars screenshoty vyvrátily config hypotézu) místo opakování deploy teorie; deník přečten PŘED diagnózou (poučení z CH-022), takže cross-site slepá ulička se neopakovala. Klíč: „Akceptujeme" komentář u známé race je vlajka — race s dopadem na UX se neakceptuje, ale řeší (grace okno je standard). **Poučení:** sliding session bez single-flightu na FE je rozbitá by-design; při rotaci tokenů vždy počítej se souběhem (taby/PWA/retry).
**Příznak:** pravidelné odhlašování ~po TTL access tokenu ač refresh „funguje"; v Network tabu ≥2 paralelní `/auth/refresh`, druhý 401 `REFRESH_TOKEN_ABUSED`; v BE logu „Reuse detection: rodina tokenů revokována" u aktivních uživatelů.

### ✅ ŘEŠENÍ — Vypravěč assety: „průhledné" PNG z generátoru měly vypečenou šachovnici; flood-fill od okrajů ji odklíčoval bezpečně · 2026-07-21
**Co nakonec zabralo:** kontrolní arch zmenšenin (24/48/64/128 px na tmavém i světlém pozadí, prohlédnutý VLASTNÍMA očima, ne domněnkou) odhalil, že všech 20 dodaných PNG je plně neprůhledných — generátor vyexportoval náhled s vypečenou šachovnicí průhlednosti (dva ploché odstíny ~240/~254). Pixelová sonda to potvrdila (alfa 255 všude). Řešení ve `scripts/vypravec-assets.mjs`: **flood-fill od všech okrajových pixelů** přes barvy blízké dvěma odstínům šachovnice (pozadí je vždy souvislé s okrajem) + 1px alpha feather proti halo. Slonovinový kabát uvnitř postavy nemá k okraji cestu přes barvy šachovnice → neukousne se, na rozdíl od globálního klíčování bílé.
**Proč to je správně:** globální práh „všechno skoro bílé → průhledné" by prostřílel díry do slonovinového kabátu Ishidy (~230–245); topologická podmínka (souvislost s okrajem) odděluje pozadí od postavy spolehlivě a deterministicky.
**Jak ověřeno:** druhý kontrolní arch — siluety rozpoznatelné ve 24 px (cylindr, lucerna), busty čisté v 64 px na obou pozadích, žádné zbytky šachovnice ani díry v kabátě; 20/20 souborů hlásí „šachovnice odklíčována".
...
### CH-135 — Bundle budget padal jen v CI: lokální build bez Sentry DSN měřil o 28 kB menší eager graf — 2026-07-23

Deploy gate spadl na `check-bundle-budget` (351,7 > 350 kB), ačkoli lokální brána před KAŽDÝM z 5 commitů hlásila 91–92 %. Příčina: `VITE_SENTRY_DSN` je jen v CI — lokálně je konstanta prázdná, `if (dsn)` je mrtvá větev a **celý @sentry/react (~28 kB gzip) se tree-shakne**. Lokální metr tedy měřil jiný bundle než CI; na stropu jsme jeli neviditelně od zapnutí Sentry (23.4) a mých +1,7 kB ho přelilo. Fix (7752f507): dynamický `import('@sentry/react')` po init + fronta raných chyb → 323,6 kB a lokální měření ≡ CI.
**Poučení / příznak:** brána závislá na build-time env měří jen tu variantu, kterou spustíš — číslo těsně pod limitem v CI + „u mě 92 %" = zkontroluj env-podmíněné importy (`if (import.meta.env.X)` + statický import = tree-shake rozdíl). Budget ověřuj s prod env kombinací (`VITE_API_URL+TURNSTILE+SENTRY_DSN`).

### ✅ ŘEŠENÍ — 26 hloubková revize „udělej vše": 5 agentů → ~70 nálezů → 5 commitů oprav+rozšíření — 2026-07-23

**Co zabralo:** revize 5 paralelními agenty (obsah/fakta · engine · pokrytí vs podklady · rozšíření · kompletnost postav) s tím, že KAŽDÝ vracel {soubor:řádek, scénář, návrh}; syntéza → 5 commitovaných bloků F1–F5 (5a13d791 · 6824994e · 21848a55 · 0e5a30ba · 5ed52b11; BE 6f9d452), každý s plnou bránou. Nejcennější úlovky, které by živý test těžko odhalil: fronta bublin na kolizních plochách (oslavy pj-start/tm-vycvik i milník „první hráč" se do té doby NIKDY nedoručily), keepalive flush mimo `/api` (ztráta telemetrie při každém zavření tabu), fixace cesty na cizí svět přes PomocnýPJ roli, `scene.created` jen z 1 ze 4 UI cest.
**Proč správně:** agenti měli ZÚŽENÁ zadání s explicitními soubory a formátem výstupu — surová data, ne eseje; obsahový agent verifikoval každé tvrzení proti docs/funkce (zdroj pravdy), ne proti paměti.
**Jak ověřeno:** 100 vitest (12 nových), build s PROD env kombinací (326,2/350 kB — poučení CH-135), BE jest+lint.
**Zhodnocení:** dobře — kolizní fronta opravila 4 nálezy najednou (jeden mechanismus); špatně — 2 agenti spadli na API stall a museli se resumovat (SendMessage z transkriptu funguje spolehlivě); test fixtury „nepokrytá routa" používaly konkrétní routu (`/ikaros/akce`), kterou revize pokryla → 2 falešné pády. Poučení: fixtury „neexistujícího" vybírat z vyhrazeného namespace (např. `/test/neexistuje`), ne z reálných rout.

### ✅ ŘEŠENÍ — 26 verifikační kolo: 3 agenti dokázali, že „hotovo" ≠ „funguje" — 2 přerušené řetězy + serializace bublin — 2026-07-23

**Co zabralo:** po dokončení F1–F5 třetí kolo agentů s jiným zadáním: NE „co chybí", ale „DOKAŽ, že každý řetěz funguje end-to-end" (UI akce → op/REST → emit → krok) + skeptik hledající interakce mezi novými mechanismy. Úlovky, které checklist-verifikace nenajde: (1) krok „Řiď scény" byl NESPLNITELNÝ — aktivace scény jde REST endpointem mimo ops choke-point, event nikdy nevystřelil; navíc po opravě by se krok plnil sám už založením scény → přepsán na skutečnou orchestraci (přiřazení JINÉHO člena ∨ deaktivace); (2) bublinaStore na klidné ploše PŘEPISOVAL (poslední vyhrává) a show() vracelo true i při dropu — replika 9 se uměla „spálit" neviděná (dismissed zapsán, obsah nedoručen). Fix: fronta všude, žádný přepis, konzumace „jednou za život" nároku přes priZobrazeni callback až při skutečném zobrazení.
**Jak ověřeno:** 105 vypravec + 794 TM/api testů (5 nových serializačních; testy PŘED fixem zůstávaly zelené — frontu zkoušely jediným prvkem), build s prod env 326/350 kB.
**Zhodnocení:** dobře — „dokaž řetěz" zadání našlo díry, které „zkontroluj seznam" zadání o hodinu dřív prohlásilo za hotové; špatně — event mapping (op→event) jsem původně napsal od stolu bez ověření, které op typy UI reálně posílá. Poučení: mapuješ-li cizí eventy/op typy, NEJDŘÍV grep skutečných producentů, pak mapa; a stavový singleton s „jednou za život" sémantikou potřebuje konzumaci při doručení, ne při odeslání.

### ✅ ŘEŠENÍ — 26 finální audit: 3 agenti (regrese · simulace 5 person · meta-konzistence) → 18 oprav — 2026-07-23

**Co zabralo:** třetí kolo s novými úhly — regresní audit NEauditovaných opravných commitů, simulace person krok-za-krokem a meta-konzistence docs/CI. Klíčové úlovky: smyčka bubliny „Postava je na světě" (vracela se navždy a krmila auto-tichý čítač — kombinace sessionDismiss čtení vs. interakce zápisu), mrtvé Měďákovo tlačítko (pointer-events:none dědičnost, jsdom to nevidí), neexistující token --acc (neviditelný focus), ztráta persona dialogu ex-anonymům (race PATCH vs. init GET) a POPISKY_CEST, které minulý skript tiše nezapsal (exit v půlce = předchozí úpravy zapsané, commit message lhala).
**Zhodnocení:** dobře — „simuluj uživatele" našel to, co „ověř seznam" ani „dokaž řetěz" nenašly; špatně — vícekrokové node skripty s exit-on-MISS uprostřed zapisují částečně (nutno buď all-or-nothing, nebo po failu znovu ověřit VŠE, ne jen zbytek). Poučení: commit message ≠ důkaz; CSS chyby (pointer-events, neexistující var) jsdom testy strukturálně nevidí — u nových interaktivních prvků grep dědičnosti pointer-events a existence použitých tokenů.

### ✅ ŘEŠENÍ — 26 páté auditní kolo: adverzariální workflow našel 3 regrese v mých vlastních opravách — 2026-07-23

**Co zabralo:** po 15 opravách 5. kola (2 vysoké — cross-account localStorage klíče, otrávení sync fronty) jsem NEspoléhal na zelené testy a spustil adverzariální **workflow** (6 skeptiků na Opusu, každý s úkolem VYVRÁTIT jednu skupinu oprav, ne potvrdit). Našel 3 regrese, které jsem sám zavedl a testy je neodhalily: (1) 4xx-drop zahazoval i přechodné 429/408/425 → za sdílenou IP ztráta onboarding progresu; (2) minAudienceNote gate skrýval poznámku právě tomu pod-prahovému publiku, pro které je psaná (porušení typového kontraktu, který říká „audience filtruje kartu, ne poznámku"); (3) changelog badge srovnávaný jen datem minul změnu téhož dne. Plus 4 rizika (busta chyby z TM mimo TM, focus na odmountovaný prvek, mrtvá dep, fresh-mount live region).
**Proč správně:** zadání „VYVRAŤ, ne potvrď" + jeden skeptik na cluster + strukturovaný verdikt DRŽÍ/ROZBITO/RIZIKO → adversariální tlak našel to, co pozitivní verifikace („ověř, že funguje") mine. Per-uid klíče (nejrizikovější změna) verifikací DRŽELY.
**Jak ověřeno:** oprava každé regrese + reálný CI test badge-hybridu (starý test díru maskoval — asertoval jen >0), 119 testů, prod build 94 %.
**Zhodnocení:** dobře — workflow s adversariálním rámováním je kvalitativně silnější než sériové „ověřovací" agenty; poprvé v celé fázi verifikace našla chyby V OPRAVÁCH, ne v původním kódu. Špatně — 4xx jsem bral jako monolit (HTTP status není binární „klient vs. server"; 429/408/425 jsou přechodné), a date-only srovnání je klasická past „ztráta rozlišení při shodě". Poučení: (a) klasifikace chyb podle status-range vždy vyjmenuj retryable výjimky; (b) když nahrazuješ index-based porovnání date-based, ptej se „co při shodě data"; (c) render podmínka MUSÍ ctít typový kontrakt pole, ne ho obejít.

### ✅ ŘEŠENÍ — 26 závěrečný podpis: 35 agentů (8 dimenzí + adverz. verifikace každého nálezu + kritik úplnosti) na „hotovém" kódu → 20 potvrzených + jedna WCAG regrese třídy CH-134 podruhé — 2026-07-23

**Co zabralo:** poslední, nejhlubší brána před GO — workflow: 8 nezávislých dimenzí objevu (a11y · stavové race · onboarding merge · registr↔realita · journey splnitelnost · eventy · docs drift · CI kontrakt), pak KAŽDÝ nález prošel adverzariální verifikací (skeptik má nález VYVRÁTIT) a nakonec kritik úplnosti („co jsme neprověřili?"). Na kódu, který už prošel 5 auditními koly, to našlo 20 reálných vad + 4 mezery úplnosti; jen 1 falešný poplach. Nejtěžší: token `--acc` nebyl NIKDE globálně definován → `var(--acc)` je invalid-at-computed-value → prohlížeč zahodí `:focus-visible` outline (WCAG 2.4.7) i `.vypravec-highlight`. To je **podruhé tatáž třída jako [CH-134](#ch-134--průhledný-panel-jsem-2×-opravoval-vrstvením-přitom-varbg-vůbec-neexistoval--2026-07-23)** (nedefinovaná custom property zneplatní celou deklaraci) — tentokrát u outline místo background. Fix: `--acc` jednou v `tokens.css :root` (vzor `--role-admin-bg`), žádné per-element hex fallbacky. Kritik úplnosti přidal ještě indexaci FAQ do fulltextu (nejbohatší korpus dosud nehledatelný) a čištění cross-world fronty bublin.
**Proč správně:** „hotovo po 5 kolech" NENÍ důkaz; každé kolo mělo jiné dimenze, a teprve dimenze „a11y computed-value" našla `--acc`. Adverzariální rámování drží falešné poplachy nízko (1/26). Per-nález verifikace = žádná oprava naslepo.
**Jak ověřeno:** 429 testů (nové: FAQ ve fulltextu, toolbox `to` routy, deep-link kotvy, cross-world fronta), tsc+eslint čisté, prod build 328/350 kB (94 %).
**Zhodnocení:** dobře — vícedimenzní sweep + adverz. verifikace + kritik úplnosti je nejsilnější brána, jakou mám; poprvé dala defensible GO. Špatně — `--acc` je CH-134 podruhé: opravoval jsem tehdy `--var-bg`, ale nezobecnil jsem „grep VŠECHNY použité tokeny proti definici", tak `--acc` proklouzl. Poučení: po každém CSS-token bugu spusť jednorázový audit „každý `var(--x)` má definici na dosažitelném rootu" napříč modulem, ne jen oprav konkrétní proměnnou; jinak tatáž třída recidivuje.

### ✅ ŘEŠENÍ — 25.1 in-app hlášení chyb — BE modul bug-reports plus Vypravěč persona-aware formulář; dvě pasti chytily testy — 2026-07-24

**Co zabralo:** Kanál hlášení = Vypravěč (menu „Nahlásit chybu" → `ChybaView`), formulář **mluví hlasem persony na řadě** (Ishida/Joe/Měďák) přes existující `mluvci` (`naTM ? 'tm' : scope`) — žádný nový stav, jen znovupoužití. BE oddělená entita `bug_reports` (bug ≠ moderace): `POST` veřejný (`OptionalJwtAuthGuard` + `@Throttle 5/min`, `reporterId` z tokenu ne z body), admin `GET`/`resolve` (`RolesGuard @Roles(Sa,Admin)`) — guardy **per-metoda**, ne na třídě (jinak by anon POST dostal 401). Discord notifikace přes globální `AlertService` s **unikátním `dedupeKey: bug:${id}`** (default klíč `info:Nový bug report` by v 10min cooldownu spolkl druhý report). Odkaz „odkudkoli" = event `vypravec:nahlasit-chybu` (VypravecRoot otevře panel rovnou na formuláři).
**Proč správně:** oddělená entita drží čistý lifecycle (new→resolved) bez GDPR/kategorií moderace; per-metoda guardy jsou jediný způsob, jak mít na jednom controlleru veřejný POST i admin GET; `url` bez query stringu = PII/GDPR pojistka.
**Jak ověřeno:** BE 8/8 e2e (anon/auth POST, 403/401, resolve, 404, throttle 429) + typecheck + lint:check; FE 2/2 vitest (persona hlášky, odeslání s kontextem) + build (tsc-b, bundle budget 93 %) + registr Vypravěče 123/123.
**Zhodnocení:** dobře — testy chytily **dvě reálné pasti dřív než produkce**: (1) nested `@ValidateNested` DTO bez `@IsDefined()` → chybějící `context` propadne validací až do Mongoose `required` = **500 místo 400** (class-validator undefined neřeší); (2) vite `define __APP_VERSION__` je build-time konstanta — v testu/SSR bare identifier → **ReferenceError** při konstrukci payloadu → tichý spád do `catch`, `api.post` se nikdy nevolal. Špatně — na `__APP_VERSION__` jsem nemyslel při psaní ChybaView (spoléhal na build). Poučení: (a) každé nested required DTO pole = `@IsDefined()` + `@ValidateNested`, jinak 500; (b) každou vite `define` konstantu čti přes `typeof X !== 'undefined' ? X : undefined` (existuje jen v buildu); (c) na jednom controlleru s mixem veřejné/admin operace patří guard na metodu, nikdy na `@Controller`.

### ✅ ŘEŠENÍ — 27.1 FE zlaté cesty Playwright: 6 pastí mock-API proti produkčnímu buildu, diagnostika přes dev-mode rebuild — 2026-07-24

**Co zabralo:** FE golden ①② běží proti mock-API (`page.route`) a `vite preview` produkčního buildu — React error boundary tam **tají příčinu** (žádný componentStack). Klíč k rychlosti: dočasný `npx vite build --mode development --minify false` → čitelné názvy v `console.error` → přesná lokalizace (`pagesDir.map`, `groupColorVarFor(undefined)`). Šest pastí tvaru mocku, každá jiná: (1) obecná routa `/pages/:slug` chytila i `/pages/directory` → objekt místo pole → `.map` crash (fix: specifická directory routa PŘED obecnou); (2) `Page` má povinná pole jako **pole** (`sections`/`galleryImages`/`accessRequirements`) — `null` → `.filter`/`.map` crash, nutný kompletní objekt dle `pages.types.ts`; (3) `page.plainText.trim()` bez guardu (PageViewer:79); (4) `Page.type` musí být **display** hodnota `'Postava hráče'`, ne URL klíč `'PostavaHrace'` — jinak PostavaLayout nedetekuje postavu a nevykreslí taby; (5) chat `/chat/groups` FE čeká **zabalený** `[{ group, channels }]`, ne ploché → `ChannelSidebar` volá `groupColorVarFor(undefined)`; (6) obecná world-by-id routa `[^/]+$` chytila i `/worlds/…`-pole-endpointy → `.find` crash (fix: přesné ID). Bonus: hod přes 3D overlay v headless FUNGUJE, jen text zprávy je `„součet hodu: …"`, ne `„Hod Kostkou"` (chybný odhad průzkumu).
**Proč správně:** dev-mode rebuild jen pro diagnostiku (pak zpět production pro paritu s CI) je nejrychlejší cesta k příčině v minifikovaném buildu; `e2e/` soubory Playwright transpiluje mimo build → mock/fixtures/testy jdou iterovat bez rebuildu (rebuild jen když měním produkční kód).
**Jak ověřeno:** oba golden ①② + smoke + persona 3× deterministicky zelené proti **production** buildu (4/4), eslint e2e čistý, bundle budget 93 %.
**Zhodnocení:** dobře — místo hádání tvarů jsem po 2. crashi přečetl `Page` interface a dodal kompletní objekt naráz; příčinu chatu `.color` našel cílený agent přesně (zabalený tvar) místo dalších kol. Špatně — obecné regexy v mocku (`/pages/[^/]+$`, `/worlds/[^/]+$`) jsou past: chytají víc, než chceš, a vrací špatný tvar → crash daleko od místa. Poučení: (a) mock routy piš SPECIFICKÉ (přesná cesta/ID) před obecnými; (b) fixture entity naplň dle jejího TS interface (povinná pole = správný prázdný tvar `[]`/`{}`, ne `null`); (c) na produkčním buildu diagnostikuj přes jednorázový dev-mode rebuild, ne hádáním; (d) očekávané texty z průzkumu ověř na reálném výstupu (`„součet hodu"` ≠ `„Hod Kostkou"`).

### ✅ ŘEŠENÍ — CI vitest flaky „document is not defined" = tiptap BubbleMenu setTimeout přežil unmount → cross-file leak; mock v setup.ts — 2026-07-24

**Co zabralo:** CI `Run vitest (shard 2/4)` občas `1021 passed` + **1 uncaught** `ReferenceError: document is not defined` (`BubbleMenuView.updateHandler` → `Timeout._onTimeout`), připsané `StyleRail.spec.tsx` — ale ten sám lokálně 6/6 zelený a celý shard 2/4 lokálně 127/127 (Windows timing). Kořen: `@tiptap/react/menus` `BubbleMenuView` plánuje `setTimeout`; `useEditor` unmount ho vždy nezruší → timer přežije teardown editoru a v CI (linux timing) se spustí během JINÉHO test souboru po jsdom teardownu → sáhne na `document`. `originated in <soubor>` ukazuje oběť, ne zdroj. Fix: v `src/__tests__/setup.ts` no-op mock `@tiptap/react/menus` `BubbleMenu` (jako už mocknutý Turnstile) — bublinové menu se v jsdom stejně nezobrazuje (nemá layout) a **žádný spec neasertuje jeho obsah** (grep přes `src`), takže mock jen odstraní zdroj timeru; `...importActual` ponechá `FloatingMenu` a ostatní reálné.

**Proč správně:** mock odstraní `BubbleMenuView` úplně → timer strukturálně NEMŮŽE vzniknout (silnější než retry, které flaky jen schová). Grep potvrdil, že nic bubble-menu neověřuje → no-op je bezpečný napříč všemi shardy. Cílený mock jednoho exportu (`...await orig()`) nezasáhne `useEditor`/`EditorContent`.

**Jak ověřeno:** typecheck čistý; shard 2/4 (místo selhání) 127/127 · 1021/1021 bez uncaught po mocku; celá sada 2729 testů (dobíhá). NENÍ z BE pushe (jiný repo) ani z 27.1 (nesahal jsem na `src`/vitest — jen `e2e/`+`docs/`).

**Zhodnocení:** dobře — reprodukce lokálně (sám soubor + celý shard) rychle vyvrátila „chyba je v StyleRailu" a nasměrovala na cross-file leak; stack `node_modules/@tiptap/extension-bubble-menu` byl jasný ukazatel zdroje. Špatně — nic (flaky nebyl z mé změny). Poučení: (a) `originated in X` u vitest uncaught = oběť async timeru, ne zdroj — hledej v node_modules stacku, kdo timer plánuje; (b) knihovny co plánují `setTimeout` mimo React lifecycle (tiptap menus, popovery) leakují napříč vitest soubory ve sdíleném procesu — mock na no-op v setupu, pokud je test neověřuje; (c) flaky „jen v CI" = zkus reprodukovat celý shard lokálně, pak porovnej timing (linux vs win).

### CH-136 — přidal jsem query hook do karty, lokální ověření minulo NEPŘÍMÝ test (jiný název souboru) → CI vitest shard 3 „No QueryClient" — 2026-07-24

**Co nefungovalo:** 27.1b přidalo `useCampaignScenarios` do `GameEventCard` (a `useAllWorldGameEvents` do `TimelineEventCard`). Lokálně jsem ověřil `npx vitest run GameEvent Timeline` (45/45 zelené) + build + dotčené — prošlo. CI vitest **shard 3** ale spadl `No QueryClient set` v `columns.spec.tsx` (EventsColumn renderuje GameEventCard **bez** `QueryClientProvider`, hook volal reálný `useQuery`). Můj filtr `vitest run GameEvent Timeline` matchuje jen soubory se slovem v NÁZVU — `columns.spec` (rodič, co kartu renderuje nepřímo) do něj nespadl, tak regrese prošla do CI. Fix: `vi.mock('@/features/world/campaign/api')` v `columns.spec.tsx` (parita se stávajícím `useGameEvents` mockem).

**Poučení:** (a) přidáš-li **query hook (useQuery) do už renderované komponenty**, grepni VŠECHNY `.spec` co ji renderují — i NEPŘÍMO přes rodiče (`EventsColumn`→`GameEventCard`), ne jen soubory se jménem komponenty; (b) filtr `vitest run <slovo>` je zrádně děravý — pro jistotu spusť před pushem **celou sadu** nebo aspoň všechny shardy, když měníš sdílenou komponentu; (c) rodičovský test, který dřív mockoval jen část API modulu, po přidání hooku z JINÉHO modulu potřebuje i jeho mock (jinak reálný useQuery → No QueryClient). Rodina „lokál zelený, CI červený" ([CH-135](#ch-135--bundle-budget-padal-jen-v-ci-lokální-build-bez-sentry-dsn-měřil-o-28-kb-menší-eager-graf--2026-07-23)).

---

### ✅ ŘEŠENÍ — 27.2 P0-③ dotyková past taktické mapy: `touch-action:none` na `.viewport` se dědil na vnořené scroll kontejnery → mapa jezdila místo scrollu panelu — 2026-07-24

**Co nakonec zabralo:** dvě vrstvy současně — (1) `touch-action: pan-y` na `.body` token karty a `pan-x` na `.strip` iniciativy (+ `overscroll-behavior:contain`), aby prohlížeč vrátil nativní scroll těch kontejnerů; (2) target-guard na začátku `useViewportPanZoom.onPointerDown` — `e.target.closest('[data-map-chrome],[role="dialog"],[role="region"],[role="toolbar"],button,input,textarea,select,a')` → `return`, takže gesto začínající na UI chrome mapy nearmuje pan/pinch. Token karta má `role="dialog"`, iniciativa `role="region"` — guard je chytí bez nutnosti měnit jejich TSX.

**Proč to je správně (a ne další variace):** kořen NEBYL v jednotlivých panelech, ale v **dědičnosti `touch-action`** — `.viewport` má `touch-action:none` (musí, kvůli vlastnímu JS pan/zoom) a CSS `touch-action` se propisuje na všechny DOM potomky, dokud si sami nenastaví jinou hodnotu. Všechny panely jsou potomci `.viewport`, takže žádný z nich nedostal nativní scroll. Řešit to per-panel (přepisovat každý scroll kontejner zvlášť) by byla nekonečná série; oprava na úrovni **scroll kontejneru + sdíleného pan handleru** pokrývá i budoucí panely. `pointercancel` je už napojený na `window` (TacticalMapView), takže nativní scroll korektně zruší rozjetý pan.

**Jak ověřeno:** staticky (řetěz `viewport touch-action:none` → potomci → chybějící re-deklarace potvrzen grepem, jediný výskyt = `.viewport`); guard cílí na reálné role kořenů (`role="dialog"`/`"region"` ověřeno v TSX). **Živé ověření na telefonu čeká na uživatele** (dotyk nejde dovřít staticky — zapsáno v acceptance spec-27.2 §6).
