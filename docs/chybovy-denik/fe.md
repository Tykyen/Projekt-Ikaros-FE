...
**Fix:** robustní mapování v `BestiePanelView` (čte `name ?? label`, `description ?? value`, coerce na `''`) + guard `(a.label ?? '').trim()` v `BestieStatblock`. Crash je render-time → záplata na obou vrstvách (zdroj + spotřebitel).
