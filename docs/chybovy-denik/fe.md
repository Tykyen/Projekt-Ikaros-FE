...
**Řešení:** `PiBestiePanel` (mapa, derivát `MatrixBestiePanel` HUD + pi stupně) v `TokenSystemSheet` + `PiChatBestiePanel` (chat, reuse mapový CSS + generický `MatrixChatBestieEditModal` se schématem `pi:token`) v `BestieRollPanel`/`BestieInstancePanel`. Mechanika identická s Matrix bestií.
