...
### ✅ ŘEŠENÍ — Komunitní kniha Ikaros: sjednocení 11 rešerší → 1 stylovaná kniha (fact-check + psaní přes agenty) · 2026-07-03
**Co nakonec zabralo:** Stejný workflow jako právní rámec, aplikovaný na 11 komunitních rešerší (body 1–11). (1) **Skeleton** = reuse `docs/pravni-ramec/build/_head.html` (CSS kontrakt beze změny, jen `<title>`), nový cover/toc/dělítka/build.sh; ORDER = 17 fragmentů + strukturní. (2) **Fact-check** = paralelní web workflow (8 agentů) → kurátorský agent přetavil 161 kB JSON do `_ledger.md` (opravy A/B/C/D + právní sladění) + `_resources.md` (~195 ověřených zdrojů). (3) **Psaní** = 5 nosných kapitol (úvod, shrnutí, stránka Komunita, komunikační manuál, akční plán) přímo + 12 přes paralelní psací dávku (agenti čtou ledger/adresář/kontrakt/vzory z disku, píší fragmenty). (4) `build.sh` → Chrome headless → PDF (8,2 MB). Uloženo i do ~/Downloads.
**Proč to je správně:** reuse osvědčeného designu/pipeline = konzistence dvou sesterských knih + reskin jen v `_head.html`; **ledger jako autorita** → konzistentní fakta napříč 17 kapitolami a zachycené REÁLNÉ chyby podkladů (AI Act není plně použitelný 2.8.2026; DrD II i Příběhy Impéria NEjsou pod CC; věk souhlasu 15; Odraz mrtvý→Larpová konference; Draak.cz→Kult Draka; GamePolis(ES)→FOR GAMES); fragment-only kontrakt → čistá sazba.
**Jak ověřeno:** `build.sh` čistý (17/17 fragmentů, PDF platný xref); QA grep = pořadí kickerů OK, 0 zakázaných top-level tagů, VŠECHNY class tokeny z kontraktu (žádná vymyšlená třída), 0 markdown artefaktů; fact-check přidal ~195 zdrojů + reálné opravy.
**Pasti:** (1) writing agenti dostávají kontrakt + vzorové kapitoly + ledger jako SOUBORY na disku (ne v promptu) — štíhlý kontext + konzistence. (2) 161 kB JSON z research NETAHAT do svého kontextu — kurátorský agent → `_ledger`/`_resources`, já četl jen ledger. (3) před tiskem zbývá ověřit pár termínů conů + publikaci Digital Omnibusu v OJ (ledger sekce B).
**Zhodnocení:** dobře — bez cyklení; paralelizace (8 research + 12 writing agentů, ~1,8 M tokenů) zvládla ~200 stran ve 2 kolech. Opakovatelný postup pro další knihy ze zdrojových PDF (paměť `project_komunitni_strategie_dokument`).
