# Vypravěč · 02 — Persona a grafika (Ishida)

Stav: podklad · 2026-07-20 · **rev. 2: persona Daidalos → Ishida rozhodnutím vlastníka (2026-07-20)** · Vazby: roadmap3 fáze 25.2 (Vypravěč), 26.1–26.5 (texty cest — blokuje je persona lock), 28.2 (vyhodnocení bety)
...
## 1. Ishida — kdo je a proč

Feature se jmenuje **Vypravěč**; postava, která v ní mluví, je **Ishida** — vlastní postava vlastníka (OC). Archetyp: **něco mezi kloboučníkem a vůdcem tajné organizace** — elegantní gentleman-showman, tajemný, ale vřelý. Kotva funguje dvakrát:
...
**Ishida je v betě plně deterministický (scripted, žádné LLM).** Persona nesmí slibovat schopnost, kterou nemá — na dotaz mimo registr vždy poctivý fallback (replika 8), nikdy nehádá. Tajemno postavy je vizuální a stylistické, **nikdy epistemické** — Ishida nemlží o faktech.
...
| Repliky | mluví **Ishida**, představuje se jménem |
| Zákaz | Ishida **nikdy** neřekne „jsem tvůj PJ" |
| Vysvětlení role PJ | *„PJ je vypravěč TVÉHO světa. Já jen spravuji dům, ve kterém všechny světy stojí."* |
...
### 1.3 Kde se persona projevuje

- **Mimo svět:** plná brand grafika — post-registrační dialog (26.4), panel Vypravěče (nahrazuje AnonStartPanel „Začni tady"), empty-states platformy.
- **Uvnitř světa:** tlumená jednobarevná silueta obarvená tokenem motivu (detail theming + lazy rozpočet: [04-architektura.md](04-architektura.md)). Siluetový podpis = **cylindr**.
- HelpPage zůstává „Ishidova knihovna" — dlouhé čtení, tisk, SEO; Vypravěč do ní deep-linkuje.

---

## 2. Hlas — style-guide (závazný)

Tón: **tykání + vzletná zdvořilost.** Ishida mluví s úctou a přátelstvím zároveň („Zdravím tě, příteli") — je to hostitel, který si hosta váží, ale sám patří do trochu jiného světa.
...
1. **Uvítání:** „Zdravím tě, příteli. Jsem Ishida — tohle místo jsem stvořil a znám každý jeho kout. Než vykročíš: chceš vést hru, hrát, nebo tvořit svět?"
2. **Empty state postav (Hráč):** „Postavu ti tady zakládá tvůj PJ — napiš mu. Až bude na světě, najdeš ji pod Moje postava."
3. **Žadatel:** „Tvá žádost leží u PJ. Dokud ji neschválí, brána zůstává zavřená — i pro mě, věř nebo ne. Stav sleduj na dashboardu světa. Čekání není chyba."
4. **Oslava prvního světa:** „Nový svět stojí — a ne prázdný. Dovolil jsem si ti předchystat pravidla, měny i kalendář. Rozhlédni se."
5. **Chybějící kotva na mobilu (slovní fallback navigace):** „Na mobilu se ozubené kolečko skrývá v menu — otevři je a povedu tě dál."
6. **Kostky zakázané:** „PJ zatím žádné kostky nepovolil. Whitelist najde v Nastavení světa — připomeň mu to."
7. **Údržba:** „Za oponou se právě přestavuje scéna. Malý moment — stránka se sama obnoví."
8. **Neznámý dotaz:** „Tenhle kout je neprobádaný i pro mě. Nahlédni do plné nápovědy — a já to předám svým lidem, něco s tím uděláme."
...
| Tvář | vlídný vědoucí úsměv se zavřenými rty (žádné tesáky), lehce zvednuté obočí, hladce oholený, **jantarové oči**, **normální lidské uši** |
| Vlasy | temně fialovočerné, kratší, sčesané na stranu |
| Oděv | dlouhý **slonovinový dvouřadý kabát** s vysokým límcem, mosazné knoflíky, tenké cyan prošití; tmavě indigová vesta; **sytě magentová jednobarevná vázanka/askot**; krémové rukavice s cyan švem; **mosazný špendlík-klíč** na hrudi |
...
1. **Persona lock:** ✅ **schváleno 2026-07-20** — Ishida + tykání + 8 few-shot replik (§2.1, „zatím" — ladění replik vyhrazeno, změny jen úpravou §2.1).
2. **Grafika:** čeká na **finální upravenou referenci od vlastníka** (splňující §3.1a); pak finalizace promptů §3.4 a generace. Engine mezitím na placeholderu.
