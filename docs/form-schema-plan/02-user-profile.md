...
> Sweep 2026-06-05 (akt. 2026-06-20). ValidationPipe = `whitelist:true, **forbidNonWhitelisted:true**, transform:true` (`backend/src/main.ts:53-56`, PC-07) → neznámá/přejmenovaná pole vrátí **400** (NE tichý drop); required pole chybějící v body → 400. Write path profilu = `PATCH /users/me` (controller `updateMe`, `users.controller.ts:70`) → `usersService.update(id, dto)` (`users.service.ts:209`) — service **nedělá** class-validator re-validaci, jen kopíruje definovaná pole; DTO dekorátory běží **jen** v ValidationPipe na `@Body()`.
