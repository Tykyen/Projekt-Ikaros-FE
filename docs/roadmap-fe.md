...
*Per-svět vizuální styl, nezávislý na globálním tématu platformy. Reuse infry z kroku 1.0 (`THEMES` registry, `applyTheme`, CSS tokeny `_shared/tokens.css`). Spec: [spec-5.0.md](arch/phase-5/spec-5.0.md) · plán: [plan-5.0.md](arch/phase-5/plan-5.0.md) · audit Matrix skinu: [audit-5.0-matrix-skin.md](arch/phase-5/audit-5.0-matrix-skin.md).*

> **Odklon od původního návrhu:** místo `applyWorldTheme` scoped na `.shell` přes `data-world-theme` se světový motiv aplikuje **přepnutím globálního `:root`** při vstupu do světa (EXIT obnoví). Důvod: portály (modaly mimo `.shell`) + eliminace kolize ornamentů. Viz spec §4.2.
...
- [x] **Sada světových motivů** — reuse 21 globálních (`scope: 'both'`) + nový **Matrix skin** (`scope: 'world'`, dimenzionální glass, port starého Matrix designu)
- [x] **Kontrast guard** — manuální audit; automatický guard v editoru = krok 5.3f
- [x] `frontend-design` audit Matrix skinu proveden
- [x] Výběr motivu v UI — ve wizardu tvorby světa; plný editor = 5.3f
