...
## Fáze 4 — Globální chat (Hospoda + Camp)
...
- [x] Sidebar: Hospoda → `/chat`; Camp I.–III. disabled položky do kroku 4.2.
- [x] Tests: BE +4 (color v sendMessage/sendWhisper), FE +37 (lib, api, komponenty).

### - [x] 4.2a Camp (`/chat/camp*`) ✅ (2026-05-16)
...
- [x] 3 nezávislé místnosti (Camp I.–III.) — víc her současně, presence per-channel
...
- [x] Tests: BE +12 (kanály, presence, environment), FE +22 (lib, api, CampHeader)

### - [x] 4.2b Camp — vazba na profil postavy ✅ (2026-05-16)
- [x] Seznam přítomných v Campu zobrazuje postavu z profilu
...
- [x] Narativní hlášky příchodu/odchodu — krčmářský tón Hospody vs. poutnický
  Camp (`presenceLine`).
- [x] Počet přítomných u Hospody / Camp I.–III. v levém nav — živě
...
- [x] §8 — seznam přítomných: Hospoda = účet, Camp = postava
...
### - [x] 4.2e Zprávy nesou identitu postavy/účtu ✅ (2026-06-18)

**Spec:** `docs/arch/phase-4/spec-4.2e.md`

Uzavírá otevřenou otázku 4.2d §8 — i **zprávy** (ne jen panel) mají jméno+avatar
dle místnosti. Dřív globální zprávy ukazovaly vždy username + iniciálu.

- [x] BE snapshot při odeslání — Hospoda = účet, Camp = postava
  (`sendMessage` + `sendWhisper`, profil z `usersService.findById`).
- [x] FE `ChatRoom` předá `resolveAccountAvatar` (fallback z presence pro
  zprávy bez snapshotu); čistá funkce `roomAvatarFor`.
- [x] Tests: BE +5 (snapshot Camp/Hospoda/fallback/whisper), FE +7
  (`roomAvatarFor`, snapshot-přednost v `MessageItem`). ⚠️ Restart BE.
...
### - [x] 6.7 Soukromé konverzace postav + osobní řazení/sbalení sidebaru

*Kanál „Postavy" bez veřejné konverzace; auto soukromá konverzace postavy (vázaná na hráče) při přidělení; osobní per-membership pořadí + sbalení sidebaru. Spec: `docs/arch/phase-6/spec-6.7.md`, plán: `plan-6.7.md`. **Hotovo 2026-06-05.***

- [x] **6.7a** BE — `ChatChannel.linkedMemberUserId` + `type:'character'`; `hasChannelAccess` PJ+ bypass pro `members`; prázdný seed „Postavy" + migrace prázdné „hráči"; `ensureCharacterChannel` na eventy `character.created/updated/converted` + `world.character.assigned`
- [x] **6.7b/c** BE — `WorldMembership.chatGroupOrder/chatChannelOrder/chatExpandedGroups`; endpoint `PATCH /worlds/:worldId/chat/my-prefs`
- [x] **6.7b/c** FE — `useChatPrefs` (optimistic + debounce), `ChannelSidebar` osobní řazení (drag pro každého) + collapse z prefs (default sbalené, aktivní override)
- [x] FE — tlačítko „+" (nová konverzace) v hlavičce kanálu trvale viditelné
...
- [x] **8.7p — JaD redesign (multipovolání + obory)** — Jeskyně a draci povýšen z 1:1 legacy na funkční tvorbu postavy: multipovolání (`jad_classes` `[{c,l,s}]`, 11 povolání + obory s prahovou úrovní výběru), úroveň = auto součet, zázemí `<select>` 16 + Vlastní, přidávatelné zdatnosti/jazyky/schopnosti (`jad_profs`/`jad_langs`/`jad_feats`), odebrané jméno/přesvědčení/hráč/pomůcky, poznámky dole; kouzla beze změny, auto-caster odvozený. Migrace legacy read-only (0 data loss). Spec: [spec-8.7p-jad-redesign.md](arch/phase-8/spec-8.7p-jad-redesign.md). 29 testů, build čistý, mobil/desktop ověřeno. (2026-06-28)
...
- [x] **10.2k — Zvuky** *(2026-05-31)* — ambient playlist scény přes op `sound.playlist` (BE-ready z prep). `AmbientSoundPanel` (PJ, vedle Kostky/Efektů/Mlhy): výběr z knihovny světa `useSounds`, pořadí, Vysílat/Zastavit (optimistic + broadcast všem na scéně). `SceneSoundPlayer` (PJ i hráč): hraje `scene.activeSoundIds` přes sdílené skryté YT IFrame jádro, autoplay gate u hráčů („Aktivovat zvuk"), mute/volume. FE patcher doplněn o `sound.playlist`. Sdílí přehrávací jádro s 13.3 + chat broadcastem. Spec: `phase-13/spec-13.3.md`, plán: `plan-13.3.md`. **Defer:** sample-přesná synchronizace pozice (YT iframe = „všem stejný track", ne stejný čas).
- [x] **10.2l — Deníky na mapě** *(2026-05-31)* — token → deník / poznámky / staty postavy (overlay z token panelu, hotové z 10.2c-edit-9bc) + **deník PJ jako samostatná stránka** `/svet/:slug/denik-pj` (PJ+, sdílí endpoint `/worlds/:id/gm-notes` s deníkem na mapě). Sdílené notebook jádro `components/notebook/` (`NotebookPaper`, `useNotebookAutosave`), PJ-only nav položka (Hra), opraven `isPJForNav` (chyběla membership větev). Opraveno chybné číslování `10.2j`→`10.2l` v dosud necommitnutých souborech. Testy: `useNotebookAutosave`, `NotebookPaper`/`NotebookStatus`, `WorldGmDiaryPage` (15). Spec: `phase-10/spec-10.2l.md`.
...
*BE `sounds` + `world-sounds` kompletní vč. nominačního workflow. Nahradil stub `SoundsPage`. Spec: `phase-13/spec-13.3.md`, plán: `plan-13.3.md`. Stavěno v 5 sub-krocích (A jádro → B databáze → C mapa → D chat → E audity).*

- [x] **13.3a — Knihovna světa:** grid karet zvuků (`GET /worlds/:worldId/sounds`), skrytý YT IFrame přehrávač, náhled (`SoundPreviewBar` s ekvalizérem)
- [x] **13.3b — Filtry:** sbalitelný filtr bar (`mediaType` / `environment` / `emotionalTone` / `intensity` / `factionStyle` / `techLevel` / `magicLevel`) + fulltext, client-side filtrace
- [x] **13.3c — Správa (PJ):** CRUD zvuků světa (`SoundFormModal`), import schválených globálních (tab Globální)
