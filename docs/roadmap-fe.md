...
*Per-svět vizuální styl, nezávislý na globálním tématu platformy. Svět má buď **preset motiv**, nebo **vlastní (custom) theme** — vlastní pozadí + plný override barev a stínů. Reuse infry z kroku 1.0 (`THEMES` registry, `applyTheme`, CSS tokeny `_shared/tokens.css`).*

- [ ] **5.0a — BE:** pole `themeId` na `World` schema + `UpdateWorldDto` + default hodnota; migrace existujících světů; `themeId` vracen v `GET /worlds/:id` i `/slug/:slug`
- [ ] **5.0b — FE infra:**
  - `Theme.scope` rozšířen o `'world'`; `listThemes('world')` vrací světové motivy
  - `applyWorldTheme(id)` — analogie `applyTheme`, ale vstřikuje tokeny pod selektorem `[data-world-theme="<id>"]` (přepis platformových tokenů jen uvnitř `.shell`), ne na `:root`
  - `worldThemeAtom` = aktivní motiv světa = uživatelský override (5.0e) **nebo** sdílený základ z `World` (`WorldContext`) jako fallback
- [ ] **5.0c — FE aplikace:** `WorldLayout` nastaví `data-world-theme` na `.shell` element; platforma Ikaros mimo `WorldLayout` zůstává na globálním `themeAtom`
- [ ] **5.0d — Vlastní (custom) theme světa:** PJ může místo presetu sestavit vlastní vzhled — **vlastní pozadí** (upload obrázku přes `POST /upload/image`) + **plný override barev a stínů** (úprava CSS tokenů světa — barevné palety, `--shadow-*`). BE: `World.themeOverrides` (mapa token → hodnota) + `themeBackgroundUrl`; FE: `applyWorldTheme` vrství overrides nad zvolený preset. Promítne se do **celého `WorldLayout` včetně světového chatu (krok 6)** a všech stránek světa — jeden zdroj vzhledu. Editor UI = krok 5.3f.
- [ ] **5.0e — Uživatelský override:** sdílený základ (5.0a/d) je default, ale každý člen si může vzhled světa pro sebe **přenastavit** — zvolit jiný preset nebo vlastní úpravy (pozadí, barvy, stíny). Uloženo per-uživatel (`worldThemeAtom` přes localStorage keyed `worldId`; persistenci napříč zařízeními přes `WorldMembership` ověřit ve spec). „Reset" vrátí na sdílený základ PJ.
- [ ] **Sada světových motivů** — rozhodnutí ve spec: reuse 21 globálních (`scope: 'both'`) vs. vlastní sada; ornamenty každého skinu originální
- [ ] **Kontrast guard** — ověřit čitelnost UI prvků (chat barvy, badge) vůči světovému pozadí
- [ ] `frontend-design` audit mezi spec a impl. plánem (skiny)
- [ ] Výběr motivu v UI → implementováno v 5.3f

### - [ ] 5.3 Nastavení světa (`/svet/:worldId/nastaveni`)

*Nahrazuje stub `WorldSettingsPage`. Stránka členěná do sekcí/tabů. Vše PJ+ (kromě Leave world = Čtenář+). BE hotový — žádné nové endpointy kromě 5.0a.*

- [ ] **5.3a — Základní info:** formulář (`react-hook-form` + `zod`) — name / popis / genre / tóny / system / kostky / maxPlayers / playersWanted; reuse polí z `/ikaros/vytvorit-svet` (krok 2.3); hero upload přes `POST /upload/image` → `imageUrl`; ukládá `PATCH /worlds/:id` — **PJ+**
- [ ] **5.3b — accessMode toggle:** přepínač public ↔ open ↔ private ↔ closed, každý režim s vysvětlením dopadu na join flow; ukládá `PATCH /worlds/:id` — **PJ+**
- [ ] **5.3c — Správa členů:** tabulka členů (`GET /worlds/:id/members`), změna role (`PATCH .../members/:id/role`), skupiny (`.../group`) a **AKJ úrovně** jednotlivým členům (`PATCH .../members/:id/akj`); hierarchie — PJ nemůže demotovat/odebrat sebe, promote na PJ ošetřit ve spec; barvy skupin přes `PUT /worlds/:worldId/settings` (`groupColors`)
- [ ] **5.3d — AKJ — úrovně viditelnosti stránek:** AKJ = stupňovaná „prověrka", která řídí, které wiki stránky hráč vidí a jak utajené jsou. PJ definuje úrovně a **libovolně je přejmenuje** dle světa (např. „Veřejné" → „Tajný spis" → „Nejvyšší prověrka"). Ukládá `WorldSettings.akjTypes` přes `PUT /worlds/:worldId/settings` — **PJ+**. Přiřazení AKJ členům = 5.3c, použití na stránce = krok 7.2e
- [ ] **5.3e — Leave world:** tlačítko „Odejít ze světa" (Čtenář+, **ne PJ** — ten musí svět předat/smazat), confirm dialog, `DELETE /worlds/:worldId/members/:membershipId` s vlastním `membershipId`, invalidace `useMyWorlds`, redirect na `/` — **uzavírá D-064**
- [ ] **5.3f — Theme světa (selektor + editor):** výběr preset motivu z `listThemes('world')` **nebo** editor vlastního theme — upload vlastního pozadí, color pickery na barvy a stíny (override CSS tokenů); živý náhled, kontrast guard hlídá čitelnost; ukládá `themeId` / `themeOverrides` / `themeBackgroundUrl` přes `PATCH /worlds/:id` (napojení na 5.0d)
- [ ] **5.3g — úklid:** `mobil-desktop` audit, `napoveda` aktualizace (nová stránka Nastavení světa + sekce Role & oprávnění — světové role)

### - [ ] 5.2 World dashboard (`/svet/:worldId`)

*Nahrazuje `MemberDashboardStub` reálným obsahem (member větev `WorldDashboardPage`). Non-member / pending větve beze změny z 2.4.*

- [ ] **Novinky světa** — `GET /news?worldId=` (vrací světové + globální); zvážit reuse `NewsCard` z kroku 3.1b (ověřit kompatibilitu schématu `WorldNews` vs. `IkarosNews`)
- [ ] **Blížící se eventy** — world-scoped seznam; ověřit ve spec, zda `game-events` filtruje per `worldId` (jinak descope na fázi 9)
- [ ] **Statistiky světa** — počet členů / novinek / eventů
- [ ] **Rychlé odkazy** — dlaždice na hlavní sekce světa (zachovat z `MemberDashboardStub`)
- [ ] **„Poslední stránky"** — **descopováno** do fáze 7 (wiki modul); zatím placeholder „dostupné s krokem 7" (nebo `favoritePageSlugs` jako náhrada)
- [ ] `mobil-desktop` audit

---

**Uzavírá dluhy:** D-064 (leave world — 5.3d).
