...
- [ ] **6.2f — Barva textu + vlastní font:** `color` a `customFont` per zpráva (z profilu uživatele), kontrast guard vůči pozadí konverzace/světa
...
### - [ ] 6.5 Správa kanálů a konverzací (PJ)

*PJ/Pomocný PJ UI nad CRUD endpointy `chat` modulu. Reference: starý Matrix `ChatGroupManager`. Pozn.: **zakládání** kanálů/konverzací vtaženo do 6.1f — zde zbývá editace, mazání, reorder.*

- [ ] Editace / mazání kanálů a konverzací
- [ ] Nastavení konverzace — `accessMode` (`all`/`roles`/`members`), `allowedRoles`, `allowedMemberIds`, pořadí (`order`)
- [ ] Reorder kanálů a konverzací (drag-drop), ikona/barva kanálu
