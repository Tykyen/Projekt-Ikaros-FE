...
### - [ ] 24.2 Ops kroky vázané na deploy — [dopad střední · náklad malý] 🟢 nasazeno 2026-07-20, zbývá smoke + 2 ověření
...
**Zbývá (vše je klik, ne práce):** ① **FE deploy znovu** — nález 2 (`cdn.jsdelivr.net`) přišel až po prvním deployi · ② workflow **Migrace nahlášených příspěvků** (dry-run; zápis jen když něco najde) · ③ workflow **Server Check** kroky 13/14 — **teprve tím je splněn bod ④ „ověřit RAM"** (nasadit limit ≠ změřit) · ④ `docker logs projekt-ikaros-be | grep CSP` — dva testovací záznamy `overovaci-test.invalid` (dokáže, že se report i **zapisuje**, ne jen odkývne) · ⑤ potvrzovací proklik 2 míst (editor stránek → Videa · chat → emoji paleta); zbytek smoke nahradil audit nad buildem, viz spec §7.
