...
**Zbývá:** ① **FE deploy** (`RECYCLE_AFTER` 200 → 50 zatím jen v repu) · ② `docker logs projekt-ikaros-be | grep CSP` — dva testovací záznamy `overovaci-test.invalid` (dokáže, že se report i **zapisuje**, ne jen odkývne) · ③ potvrzovací proklik 2 míst (editor stránek → Videa · chat → emoji paleta); zbytek smoke nahradil audit nad buildem, viz spec §7 · ④ ⏳ **druhé měření RAM za pár dní** (`server-check.yml`) — bez druhého bodu není trend.
