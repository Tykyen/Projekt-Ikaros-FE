...
### - [x] 27.3 Scope registr A/B/C + scope freeze — [dopad vysoký · náklad malý] ✅ 2026-07-24 (spec-27.3; FE, čeká živé ověření badge + deploy)
...
**Hotovo (2026-07-24, FE-only):** **scoped** klasifikace (in-world nástroje zavedených PJ se neschovávají — C jen pro už-blokované věci, žádná regrese). SSOT `src/shared/scope/scope.ts` (`PREVIEW_FEATURES`/`HIDDEN_FEATURES` + helpery) → zrcadlo `docs/scope-registr.md` + průřezový koncept v `docs/funkce/00-prehled.md`. `<PreviewBadge>` (⚗, reuse `Badge`) zapojen do platform nav (Společná tvorba, Nábory, Voice krčma, 3× camp) + world nav (11 hloubkových nástrojů, 6 render míst desktop+mobil). C mechanismus = existující `flagGate` (aktivní jen „RPG systémy"/25.8, beze změny). Přesun tieru = 1 řádek. tsc+eslint+vitest 14/14+build 94 % zelené. **Návazně (mimo zátah):** B2 = default-hide B nástrojů v novém světě přes `hiddenNavItems` (BE seed) → skutečné „−20–35 % šíře"; tvrdé skrytí voice/campů = produktové rozhodnutí (1 řádek).
