...
**Hotovo (kód + ověřeno):** ① BE CI job `backend-e2e` (mongodb-memory-server, maxWorkers 1) · ② route scanner resolve konstant/base helperů (**62→0** falešných pozitiv) + `--ci` gate zadrátovaný do crossrepo jobu · ③ deploy gate „green CI for HEAD" (gh api) v obou deploy.yml · ④ release evidence (sha/ref/čas/actor → step summary) v obou deploy.yml. Chybový deník `be.md` 2026-07-19.
