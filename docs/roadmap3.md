...
### - [x] 27.2 Mobilní core cesty — [dopad střední · náklad střední] ✅ 2026-07-24 (kód; čeká živé ověření na telefonu)
**Cíl:** zlaté cesty ①–③ projít na telefonu (hráči u stolu drží mobil). Statická CSS review + živý průchod uživatelem (fb_tests_live); opravit, co drhne. 🔁 reuse 17.4 dotyk mapy. **Ne** nový mobilní layout — jen průchodnost core cest. **Spec:** `docs/arch/phase-27/spec-27.2.md`.
**Hotovo (2026-07-24, kompletní zátah bez dluhu):** statická CSS review 3 cest (fan-out per cesta) → **2 P0** opraveny — ① wizard „Co chceš vytvořit?" scrolluje (`max-height:100dvh`+`overflow-y`); ③ dotyková past mapy (`touch-action:none` z `.viewport` se dědil na scroll kontejnery) přes `pan-y`/`pan-x` na těle karty + liště iniciativy **+ target-guard** v `useViewportPanZoom.onPointerDown` (přeskočí `[role=dialog/region/toolbar]`/button/input; token karta, iniciativa i oba docky pokryty ARIA rolemi). Dotykové terče sjednoceny: cesty ①② (wizard, editor, 🎲, composer, dice picker vč. iOS anti-zoom + mobilní ✕) + **vlna 24 combat/bestie skinů** — akční terče (HP +/-, hody, iniciativa, útok/obrana, add/del, inputy) ≥40px, husté herní mřížky (pip/dot/box/wcell/pipSq/toggle) ≥24px (40px by je rozbil = vědomé UX, ne dluh). P2 úklid: mrtvý CSS `PostavaPanel`, breakpoint 720→768, `MessageItem` hover gutter, kreslicí/měřicí terče. `✅ ŘEŠENÍ` P0-③ v chybovém deníku. **Zbývá:** živý průchod na telefonu (fb_tests_live) — hlavně P0-③ dotyk (staticky nedovřen) + vzorek 3 skinů (Drd16 combat/D&D bestie/Shadowrun).

### - [x] 27.3 Scope registr A/B/C + scope freeze — [dopad vysoký · náklad malý] ✅ 2026-07-24 (spec-27.3; FE, klasifikace+freeze; ⚠️ vizuální „preview" štítek SUNDÁN — vzhled zamítnut)
...
**Hotovo (2026-07-24, FE-only):** **scoped** klasifikace (in-world nástroje zavedených PJ se neschovávají — C jen pro už-blokované věci, žádná regrese). SSOT `src/shared/scope/scope.ts` (`PREVIEW_FEATURES`/`HIDDEN_FEATURES` + helpery) → zrcadlo `docs/scope-registr.md` + průřezový koncept v `docs/funkce/00-prehled.md`. C mechanismus = existující `flagGate` (aktivní jen „RPG systémy"/25.8, beze změny). Přesun tieru = 1 řádek. **⚠️ Vizuální „preview" štítek (`<PreviewBadge>`) byl po nasazení SUNDÁN (2026-07-24, vlastník: „vzhledově hrozné") — komponenta smazána, badge odstraněn ze 7 render míst, changelog záznam zrušen. Registr + freeze pravidlo zůstávají jako klasifikace; třída B se v UI vizuálně neodlišuje.** Případný redesign štítku (`frontend-design`) = navazující volitelná práce. **Návazně (mimo zátah):** B2 = default-hide B nástrojů v novém světě přes `hiddenNavItems` (BE seed) → skutečné „−20–35 % šíře"; tvrdé skrytí voice/campů = produktové rozhodnutí (1 řádek).
