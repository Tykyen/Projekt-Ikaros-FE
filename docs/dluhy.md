...
**🔴 KRITICKÉ:**
- ~~**Refund ztratí peníze při pádu (DUR)**~~ **✅ OPRAVENO 2026-07-12:** `refund()` obalen do `session.withTransaction` (flip statusu + kredit commitnou společně, nebo rollback → status zpět `active`) = zrcadlo `purchase` RC-E5; sekvenční fallback s kompenzací pro single-instance bez replica setu; kredit přes novou `creditInSession` (bez permission gate, session-threading). Odebrání z inventáře zůstává best-effort po tx (nekritické). 5 nových testů (tx cesta + fallback + kompenzace + double-refund + creditInSession). Zbývá: `purchase`/`transfer` bez idempotency-key (double-click = 2. odečet) → nový dluh D-PURCHASE-IDEMPOTENCY níže.
- ~~**Klient je autorita nad hodem (GI)**~~ **✅ OPRAVENO 2026-07-12 (Cesta A):** sdílený `common/dice/dice-payload.validator.ts` `sanitizeDicePayload` volaný z `chat.service.ts` i `map-operations.service.ts` (dice.roll) — u součtových typů PŘEPÍŠE `sum`/`total` z `faces` (klientův `total:999` zahozen), ověří meze faces + cap, `400 DICE_PAYLOAD_INVALID` při nesmyslu; systémové typy (2d6+/roll-under/percentile/success-pool/mixed/flat) validují meze + rozsah. 26 unit testů. **Zbývá follow-up (Cesta B):** re-rolling (hráč hází lokálně dokola) + cílené podvody uvnitř systémových hodů + kosmetická pod-pole (tens/ones/crit) → potřebovaly by server-authoritative RNG (port enginu). Viz nový D-DICE-SERVER-RNG níže.
...
### D-PURCHASE-IDEMPOTENCY — Nákup/převod bez idempotency-key (double-click = 2. odečet)
**Soubory:** BE `campaign-purchase.service.ts` `purchase()`, `character-accounts.service.ts` `transfer()`.
**Stav:** nákup i převod jsou atomické (tx/kompenzace), ale **nemají idempotency-key/nonce** — dva rychlé identické requesty (double-click, retry) provedou **dva** odečty z jednoho záměru. Chat `sendMessage` má `clientNonce` (sparse unique index) jako vzor.
**Trigger:** hlášení dvojitých odečtů, nebo příprava na horizontální škálování.
**Co bude potřeba:** FE pošle `clientNonce` (UUID), BE drží unique index a 2. request se stejným nonce vrátí 1. výsledek místo dalšího odečtu. Vzor `clientNonce` z chatu.
...
**Stav:** GI díra „klient je autorita nad total" **zavřena Cestou A 2026-07-12** (server přepočítá `sum`/`total` z faces, meze). NEzavřené zbytky: (a) **re-rolling** — hráč hází lokálně dokola a pošle „šťastný" ale validní hod; (b) **cílené podvody uvnitř systémových hodů** (2d6+/roll-under/percentile/mixed), kde server neumí přesně přepočítat bez portu enginu; (c) **kosmetická pod-pole** (d100 `tens`/`ones`, `crit`, `breakdown`) se nevalidují proti faces.
**Trigger:** potřeba kompetitivní/turnajové integrity (dnes hobby + moderující PJ = přijatelné).
**Co bude potřeba:** Cesta B — FE pošle **záměr** (typ, count, sides, modifier, target, threshold), server hodí svým RNG a vrátí kompletní payload; FE 3D přehraje předurčený výsledek přes `@` notaci (`diceNotation.ts` to umí). Vyžaduje port roll enginu na BE (fate/exploding/pool-hits/roll-under/percentile/mixed) — pozor na `2d6+` (`sum≠Σfaces`) a nesčítání modifieru u GURPS/CoC/flat (drift risk vs. historické hody).
