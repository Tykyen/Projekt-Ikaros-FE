# M-SCAN — integrity-scan (tvrdý důkaz DB integrity)

> **READ-ONLY** sken databáze. Žádný `write`/`update`/`delete` — jen `find`/`countDocuments`/`aggregate`.
> Bezpečné i proti produkci. Měří **5 tříd** najednou: **TYPE** (typová mina), **OR** (orphans),
> **RR** (broken refs), **DUP** (duplicity), **INV/CARD/STATE/SET** (invarianty).
>
> L4 důkaz pro [`../../db-integrity-audit.md`](../../db-integrity-audit.md): čtení kódu říká „validace tu
> chybí" (L2), tenhle sken řekne „**v DB teď leží N nekonzistentních záznamů**" — reálný dopad.
> Sourozenec [`cascade-delete-plan/tools/orphan-scan.md`](../../cascade-delete-plan/tools/orphan-scan.md), ale širší (všechna FK pole + dup + type + invarianty).

## ⚠️ Pořadí čtení výsledku

1. **TYPE blok první.** Pokud hlásí mix `objectId`+`string` u nějakého pole, **orphan/ref čísla u těch
   kolekcí jsou nespolehlivá** (string set nezmatchuje ObjectId → false-negative). Srovnej typ, pak znovu.
2. **0 všude** = integrita _v praxi_ čistá (data malá / migrace zatím nerozbila). Kód ale pořád chybí
   validaci → nález platí jako **riziko**, jen „nevybuchlo".
3. **N > 0** = přímý důkaz nálezu reálnými čísly.
...
MONGO_URI="mongodb://...<connection z BE .env>..." node integrity-scan.mjs
# jen migrovaný svět (až bude podpora WORLD_SLUG doplněna do filtrů):
WORLD_SLUG="matrix" MONGO_URI="..." node integrity-scan.mjs
```

⚠️ **Bezpečnost:** jen čte (`find`/`aggregate`/`countDocuments`). Žádný zápis. Produkční cíl je
`www.projekt-ikaros.com` ([paměť server-swap]) — read-only to drží bezpečné, ale **nejdřív dev/staging
nebo svět `matrix`**. Pozn.: `idSet` načítá `_id` do paměti — na velké kolekci (`chatmessages`) zvaž
projekci/stream nebo zúžení `WORLD_SLUG`.

## Vazba na typovou minu

Skript úmyslně tiskne **TYPE blok jako první**. `custom_emotes.worldId` jako `objectId` mezi `string`
ostatních = mina: `countDangling('custom_emotes', 'worldId', worldIds)` (string set) by emoty
prohlásil za orphany **i když nejsou** (false-positive) — nebo naopak minul reálné. **Než se typ
srovná, ber `custom_emotes` řádky s rezervou.**

## Blob / Cloudinary

Integrity-scan nevidí externí úložiště — blob leak řeší [`cascade-delete-plan/tools/orphan-scan.md`](../../cascade-delete-plan/tools/orphan-scan.md) (sekce Blob audit). Tady jen DB.
