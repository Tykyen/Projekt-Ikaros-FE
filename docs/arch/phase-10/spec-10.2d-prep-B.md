...
>
> **Revize 2026-06-18 (výřez obrázku — parity s Page/GameEvent/WorldNews):** `BestieEditorModal` teď předává `HeroUploadCard` i focal/zoom/fit handlery → uživatel klikem volí viditelnou část obrázku (focal bod), zoom 100–400 %, fit cover/contain. Nová pole `imageFocalX/Y`, `imageZoom`, `imageFit` (default null = 50/50, cover) napříč FE (`bestiar/types.ts`, BestieEditorModal, BestieCard přes `getImageStyle`) i BE (`bestie.interface.ts` + schema + Create/Update DTO + `toEntity` mapper). Zobrazení výřezu: **BestieCard** (katalog), **PIXI token na mapě** (`TokenSprite` — cover/contain + zoom + focal přes sprite `scale`+`position` v kruhové masce, místo dřívějšího roztažení na čtverec; resolver `resolveTokenImageCrop` z `lookupBestie`) i **`PaletteAvatar`** (24px, přes `getImageStyle`). Migrace netřeba — pole optional, default null = cover 50/50.
