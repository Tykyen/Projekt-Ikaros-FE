...
Vynechává se: **Výdělek** (mzdy, není zboží), **Pomocné tabulky** (jen legenda), listy vzhledů (zdroj obrázků).

**Vědomé opravy vs. zdrojový Excel** (ekonomická revize cen + mezd, schváleno 2026-07-13): kráva 400 → **150 zl** (vybočovala: vůl 100, kůň tažný 600) · sójový olej 1000 → **10 zl** (překlep; sója 3 zl) · druhá „Destilační kolona (základní)" → **„(pokročilá)"** (stejný název, dvě ceny). Zbytek ověřen: 1000/1000 cen sedí na Excelem počítané sloupce Slunce/Měsíc; mzdy (Výdělek) ↔ jídelní tiery ↔ ceny služeb jsou konzistentní (platnéř 37 dní = rytířská zbroj; mág 10 h = očarování). Neopraveno v Excelu (mimo seed): Theurg měsíčně 100 místo 14 400 — vzorec si opraví autor. Řádky sekčních nadpisů a obecných druhů („Místní pivo"…) → sekce „Obecné" nebo sekční nadpis dle listu. Celkem ~1 014 položek ve 20 cenících + ~103 předmětů v katalogu Předmětů (zbraně+zbroje, systemId `matrix`).
