...
3. **F5 — Odkazy** (spec [`f5-links.md`](./f5-links.md)). Stav:
   ...
   - 🟡 **1b broken-fix** (`fix-matrix-links.yml` + `migration/f5-fix.js`): 1. běh opravil **content** (`stranek 46, odkazu 74`). ⚠️ Pak odhaleno, že mismatch odkazy jsou i v **Page.table** (~33, datové tabulky) a **Page.akjTabs.contentOverride** (~41, F4d záložky). Workflow ROZŠÍŘEN na všechna 3 místa (per-pole záloha, idempotentní). **NUTNÝ 2. běh fix** (dořeší table+akjTabs; content se znovu nedotkne). Audit: 531 broken (3,7 %), ~261 pravidla→Pravidlová kniha, ~157 long-tail červené.
