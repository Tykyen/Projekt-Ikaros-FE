# HANDOFF — Migrace Matrix → Ikaros (stav 2026-06-08, po F7 + F6 příprava)
...
## 🔧 F6 PAVUČINA — workflow HOTOVÝ, ČEKÁ SPUŠTĚNÍ (uživatel)
Gating otázka HANDOFF vyřešena: **cílová feature EXISTUJE** (Ikaros fáze 11.1 „Pavučina", route `/svet/:w/pavucina`, BE modul `campaign`). → F6 je čistá datová migrace (NE FE feature). Spec [`f6-pavucina.md`](./f6-pavucina.md), lokálně otestováno (mock).
- **Data:** 99 subjektů + 113 vztahů + 15 linek + 1 quick-note (tvar 1:1, 0 dangling reference).
- **Mapování:** worldId string, `_id` zachováno (reference sednou), ownerId=Tyky + isShared=true, `linkedCharacterSlug`=linkedPageSlug (PC/NPC), avatar dynamicky, slug-drift přes ALIAS.
- **Soubory (BE repo):** `migration/f6-build.mjs` + `f6-campaign.json(.gz)` + `f6-import.js`, workflow `import-matrix-campaign.yml` (dry-run/import/rollback).
- **Spustit:** dry-run → zkontroluj „NESEDI" slug výpis (doplň ALIAS v `f6-build.mjs` + `node migration/f6-build.mjs` re-build) → `import` (confirm IMPORT). Ověř naživo `/svet/matrix/pavucina` (jako PJ Tyky).
...
|---|---|---|---|---|
| ~~F6~~ 🔧 | ~~Pavučina~~ **workflow HOTOVÝ, čeká spuštění** — viz sekce „🔧 F6 PAVUČINA" výše | `Campaign{Subjects 99, Relationships 113, Storylines 15, QuickNotes 1}` (velikosti v index = **bajty**, ne docs) | `campaign` modul (existuje, fáze 11.1) | Feature ověřena = čistá migrace. Spec `f6-pavucina.md` |
...
2. ~~**F6 Pavučina**~~ ✅ ověřeno (feature existuje) → **workflow HOTOVÝ, čeká spuštění** (sekce „🔧 F6 PAVUČINA" výše).
