# HANDOFF — Migrace Matrix → Ikaros (stav 2026-06-09, po F10 + rulebook grafika)

> **DALŠÍ = F11 CHAT** (poslední F bod). Podklady připravené: [`f11-chat.md`](./f11-chat.md) — zdroj (5 kolekcí, reálné objemy), cílový model, mapování, gotchas (ChatChannels._id stringy→přemapovat, senderId→F1, GDrive→Cloudinary, color hex→slot). Postup vzor F6-F10.
>
...
| **F11** | Chat → [`f11-chat.md`](./f11-chat.md) | `ChatMessages` **1506**, `ChatChannels` **43**, `chatGroups` **7**, `ReadStatuses` **299**, `CustomEmotes` **1** (25M = velikost ChatMessages.bson, ne docs) | Chat (ChatGroup/ChatChannel, spec 6.x) | **PODKLADY HOTOVÉ.** senderId 19/19→F1, channelId 0 dangling. Gotchas: ChatChannels._id stringy (přemapovat+mapa do channelId/messages), GDrive ikony→Cloudinary, color hex→slot, PascalCase→camel |
