...
3. **F5 — Odkazy** (spec [`f5-links.md`](./f5-links.md)). Stav:
   - ✅ **1a runtime rewrite** (FE hook `useBrokenLinks`): interní odkazy (holý slug / `/slug`) → world-scoped `/svet/matrix/<slug>` + SPA navigace; externí nedotčené; neexistující červené. **NASAZENO + OVĚŘENO naživo 2026-06-08** (Cael→Švédsko: refresh i Back OK). ⚠️ Race fix: hook sleduje obsah `MutationObserver`em + `pageshow` bfcache (jinak odkazy nahodile — TipTap/tabulka renderují async). 9/9 testů, build OK.
   - ✅ **1b broken-fix** (`fix-matrix-links.yml` + `migration/f5-fix.js`): SPUŠTĚNO → **content 74 odkazů opraveno** (`abigail-wattson`→`abi`…). Workflow řeší content+table+akjTabs (per-pole záloha, idempotent, IIFE kvůli mongosh scope). ⚠️ **Table + akjTabs na serveru měly 0 mismatchů** — ověřeno diagnostikou: profilové stránky existovaly PŘED F4a/F4b importem (`if(ex)skip`), takže mají vlastní správné tabulky; mé lokální `.json` (33 table odkazů) se na ně nepřepsalo. Server = autorita. Audit: 531 broken (3,7 %), ~261 pravidla→Pravidlová kniha, ~157 long-tail ponecháno červené (hook 1a je značí).
   - ✅ **Deploy/cache fix** (souvisí): FE/BE deploy padal na `docker compose build --no-cache` (OOM/broken pipe) → odstraněn `--no-cache` + SSH keepalive. `nginx.conf` SPA cache: `index.html`+`sw.js` no-cache, hashované assety immutable (jinak hráči viděli starou verzi po deployi). SW (`public/sw.js`) je jen push, necachuje.
   - ✅ **2 auto-link** (hook `useAutoLink`, spec [`f5-autolink.md`](./f5-autolink.md)): plain-text zmínky entit (Lokace/Ostatní/PostavaHrace/NPC) → world-scoped `<a data-autolink>`, nav dodá useBrokenLinks. Filtr vlastních jmen (víceslovné | jednoslovné ≥5 & ne blacklist+`matrix`), bez skloňování (1. pád), case-sensitive, 1. výskyt/stránka, ne self, jen content (ne tabulky). Audit: 707 odkazů/522 stránek (filtr odřízl 4264 šumu jako `Nebo`). 17/17 testů, build OK. **NASAZENO?** — čeká deploy+ověření.
4. **F12 — Obrázky** → **PODKLADY HOTOVÉ: [`f12-images.md`](./f12-images.md)** (rozsah 2389 imageUrl GDrive ID + 3 znaky frakcí; obrázky teď ROZBITÉ; 🔴 povinně webp; Cloudinary upload z GDrive URL; mechanika Node+SDK ne mongosh; otevřené otázky). Začni tím dokumentem.
