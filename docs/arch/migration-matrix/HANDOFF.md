...
3. **F5 — Odkazy** (spec [`f5-links.md`](./f5-links.md)). Stav:
   ...
   - 🔜 **1b broken-fix** (`fix-matrix-links.yml` + `migration/f5-links.json.gz`): 14 postava-mismatch mapování / 76 odkazů (`abigail-wattson`→`abi`…). ČEKÁ dry-run → fix. Audit: 531 broken (3,7 %), z toho ~261 pravidla→Pravidlová kniha, ~157 long-tail ponechat.
