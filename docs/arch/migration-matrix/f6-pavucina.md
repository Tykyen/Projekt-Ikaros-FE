...
2. **`ownerId` = Tyky (hlavní PJ), `isShared = true` pro VŠECHNY záznamy.** Důvod: pavučina je PJ nástroj; Tyky (PJ) vidí vše tak jako tak, `isShared:true` zpřístupní i JohnAdminovi (PomocnyPJ). Zachovat 4–5 původních Matrix vlastníků = zbytečná fragmentace (žádný by neviděl celý graf) + nutnost F1 mapování. *(Alternativa: mapovat původní ownery přes F1 user-map — zamítnuto pro PJ-nástroj.)*
...
`dry-run` (default) / `import` (confirm `IMPORT`) / `rollback` (confirm `ROLLBACK`). mongosh přes SSH, world `6d6174726978000000000001`, IIFE (mongosh gotcha), `ownerId` = lookup Tyky z `db.users.findOne({email})`. 
...
- Tyky `_id` existuje v prod `users`.
