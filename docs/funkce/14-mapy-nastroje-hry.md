...
- **Co jde dělat:**
  ...
  - **Scroll panelů prstem (27.2)** — tělo token karty (`.body`) a lišta iniciativy (`.strip`) se na dotyku posunou; dřív `touch-action:none` z `.viewport` bránil nativnímu scrollu → místo scrollu jezdila mapa pod panelem (P0). Opraveno `touch-action:pan-y`/`pan-x` na scroll kontejnerech.
...
- **Zvláštnosti:** disambiguace token vs. pan přes **dvojitý gate** (na pointerdown i pointermove) — deterministické bez ohledu na pořadí PIXI federated vs. DOM eventů. `touch-action:none` na `.viewport` (nutné pro vlastní gesta). Unit testy pokrývají 1-prst pan, gate, pinch+pan anchor. **27.2:** `touch-action:none` se dědí na vnořené scroll kontejnery → panely bez vlastní deklarace přišly o nativní scroll; opraveno `pan-y`/`pan-x` na `.body`/`.strip` + **target-guard** v `useViewportPanZoom.onPointerDown` (`e.target.closest('[role=dialog/region/toolbar],button,input,textarea,select,a,[data-map-chrome]')` → gesto na UI chrome nearmuje pan/pinch; token karta=dialog, iniciativa=region, docky=toolbar). Dotykové terče akcí sjednoceny ≥40px napříč cestami ①–③ i všemi 24 combat/bestie skiny (husté herní mřížky pip/box/dot ≥24px — vědomě NE 40px, aby se mřížka nerozpadla).
