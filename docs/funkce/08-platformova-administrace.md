...
- **Notifikace (2026-07-04):** nová zpráva → web push (PWA, kategorie `adminChat`, default ZAP, opt-out v profilu → Nastavení notifikací → „Správa platformy") + in-app badge „Chat správy" v Administraci (WS `platform-chat:activity` → `user:{id}` příjemců mimo odesílatele; efemérní counter, reset při vstupu na /admin/chat). Příjemci: `all` → všichni Sa+Admin, `members` → `allowedMemberIds`+Superadmini. Perzistentní unread = follow-up.
