...
- **Skiny per motiv: 21/21 platformových motivů** v `komunitniBestiarSkins.css` (globální CSS, ne module; scoped přes `[data-theme='…']` + data-atributy kostry `data-bestie-book/-portrait/-ruleset-tabs/-statblock/-lib-tab/-lib-row/-discussion`). Importuje `KomunitniBestiarPage.tsx:18` i `KomunitniBestieDetailPage.tsx:26`. Neutrální motivy (`bila`/`modre-nebe`/`zlaty-standard`) záměrně minimalistické (signature = čistota).

**Hranice / co neumí:**
- `mobil-desktop` audit **neproběhl naživo** — ověřeno jen staticky (CSS má `@media`).
...
**Stav:** ✅ kód kompletní (BE 12/12 testů, FE build ✓; skiny 21/21 motivů v `komunitniBestiarSkins.css`). Zbývá jen `mobil-desktop` naživo a živé ověření (mimo kód).
...
**Co jde dělat (vše):** 2 knihovny + filtry vzácnost/štítek · karta rostliny · tvorba (pole + obrázek přes `HeroUploadCard`) · úprava všech polí (autor/kurátor) · schválení (kurátor) · smazání (autor draftu/kurátor) · pending fronta · **vklad do obchodu světa** (Etapa B): **po jedné** z detailu rostliny (tlačítko „＋ Vlož do obchodu" → 1 `ShopItem`: name = jméno rostliny, description = souhrn Roste/Použití/Lidová jména, obrázek z rostliny, cena předvyplněná `suggestedPrice ?? 0`) i **hromadně** z listu (tlačítko „🛒 Vlož vše do obchodu" → bulk endpoint za všechny zobrazené/filtrované rostliny, jednotná výchozí cena, dávky ≤200; PJ pak dolaďuje ceny v obchodě) · **nahlásit rostlinu** (`ReportButton targetType="plant"`, `KomunitniPlantDetailPage.tsx:106`). (`useKomunitniHerbarMutations.ts`, `components/PlantEditorModal.tsx`, `components/InsertToShopModal.tsx`, `api/herbarApi.ts`)

**Moderace (20B, D-070 uzavřen 2026-07-12):** report/hide rostliny je napojen na moderační pipeline — `PlantsModerationEnforcementListener` (`modules/plants/moderation-enforcement.listener.ts`) reaguje na `targetType === plant`: M2/M3 → `moderationHidden = true` (revert → `false`), M4 → **hard delete** (rostliny nemají soft-delete, revert je nevratný — jen log), M5–M7 ignoruje (account-level řeší modul `users`). Best-effort — chyba nikdy neshodí `resolveReport`. Skryté rostliny vidí v listu i detailu jen Admin+ (`plants.service.ts:33-56`).
...
**Zvláštnosti:** Kurátorská množina rolí sdílená s bestiářem (`isBestieCurator`). `PlantsRepository.toEntity` = explicitní mapování polí. `PlantsModule` silně zjednodušen oproti bestiae (jen community scope, žádný gateway/komentáře/statbloky). **Úklid Cloudinary blobů (D-071 uzavřen 2026-07-12):** hard-delete rostliny i výměna obrázku emitují `media.orphaned` (`plants.service.ts:111`, `:155`, `:180`) — žádný osiřelý blob. **Vzácnost lze v editu vymazat (D-072 uzavřen 2026-07-12):** FE posílá `rarity: null` jako sentinel „vymazat na neurčeno", BE ho překládá na `$unset` (enum v schématu `null` nezná — `plants.service.ts:113-119`). ⚠️ Komentářové hlavičky FE souborů omylem uvádějí „21.5b" (jde o 21.5a).

**Migrace (počáteční náplň):** seed skript `backend/scripts/seed-plants/index.ts` (npm `seed:plants`) — načte `plants.json` (**56 rostlin** + obrázky) z `C:/Matrix/ProjektIkaros/migrace-herbar/` (mimo repo, override `HERBAR_DIR`), nahraje obrázky na Cloudinary jako WebP (folder `community-herbar`) a vloží `scope:'community'`, `status:'approved'`, autor Superadmin (Tyky). Idempotence dle `name+scope`; podporuje `--dry-run`/`--limit`/`--export`. **OSTRÝ běh proběhl 2026-07-20** — všech **56 rostlin** nalito do produkce (roadmap3 karta 24.4, potvrzeno uživatelem). Pozn.: stav nasazení nelze ověřit z kódu — u budoucích seedů ověřovat zvenku (GitHub Actions / živý katalog / `/api/health` `version`), ne z poznámek (CH-126).

**Stav:** ✅ (Etapa A+B + report/moderace/orphan cleanup 2026-07-12 + ostrý seed 56 rostlin 2026-07-20) — sladěno s kap. 04 „Herbář — komunitní katalog rostlin (21.5a)". Odložené rozšíření (**skiny 21 motivů**, **per-systém staty**) viz „Hranice / co neumí" — nejde o nedodělek Etapy A/B.
**Kód:** FE `features/ikaros/herbar/` (`KomunitniHerbarPage.tsx`, `KomunitniPlantDetailPage.tsx`, `components/PlantEditorModal.tsx`, `components/InsertToShopModal.tsx`, `hooks/{useKomunitniHerbar,useKomunitniHerbarMutations}.ts`, `api/herbarApi.ts`, `types.ts`, `herbarSkins.css`), `router.tsx:196-197`, `SpolecnaTvorba/tiles.ts:72-79`. Obchodní strana `features/world/shop/` (`types.ts`, `api.ts` `useBulkCreateShopItems`, `components/{ShopItemForm,ShopItemCard}.tsx`). BE `modules/plants/` (`plants.{controller,service,module}.ts`, `repositories/plants.repository.ts`, `schemas/plant.schema.ts`, `interfaces/plant.interface.ts`, `dto/{create,update}-plant.dto.ts`, `community-plant-review.provider.ts`, `moderation-enforcement.listener.ts`), `pending-actions/pending-action-type.enum.ts:40`; bulk + obrázek `modules/campaign/` (`campaign.controller.ts:612` bulk, `campaign.service.ts:1036` `createShopItemsBulk`, `repositories/campaign-shop-item.repository.ts:41` `createMany`, `dto/bulk-create-shop-items.dto.ts`, `schemas/campaign-shop-item.schema.ts:25-29` image pole). Seed `backend/scripts/seed-plants/index.ts`. Spec `docs/arch/phase-21/spec-21.5a-herbar.md`.
...
2. ✅ VYŘEŠENO 2026-07-13 → route smazána 2026-07-18 (D-DATA-SYNC-ZBYTKY a) — **Dva zdroje adresáře postav.** FE už legacy `GET /characters/directory` nevolá vůbec: `useCharacterDirectory` je adapter nad `GET /pages/directory` (`characterId` + `ownerUserId` v entry, anonym smí veřejný svět). Viditelnost karet jednotně řídí page-level visibility. HTTP route `characters.controller.ts` (`GET /worlds/:id/characters/directory`) + import `OptionalJwtAuthGuard` **smazány** — bylo ověřeno grepem 2026-07-17: nula callerů ve FE `src`, e2e i BE testech. **`findDirectory` (repo) i `characters.service.getDirectory` ZŮSTALY** — visí na nich enrich portrétů v `chat.service`. ⚠️ Pořadí deploye: BE bez route až po tom, co je naživo FE bundle s adapterem (commit `368d3c78`+), jinak starý bundle dostane po smazání 404.
