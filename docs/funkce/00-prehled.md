...
- **Scope registr A/B/C + freeze (R3 27.3):** každá viditelná plocha (vstupní bod v UI) je zařazena: **A** = beta core (certifikováno zlatými cestami 27.1, plný support) · **B** = funguje, ale okrajové/hloubkové · **C** = experimentální/blokované → skryté za feature flagem (nav hide + route `flagGate`). **SSOT** = `src/shared/scope/scope.ts` (`PREVIEW_FEATURES`/`HIDDEN_FEATURES`), lidsky čitelné zrcadlo `docs/scope-registr.md`. **Scoped, ne destruktivní:** in-world nástroje zavedených PJ se neschovávají (C jen pro už-blokované „RPG systémy", 25.8). Nová viditelná doména se do bety přidává jako C/B, ne rovnou A (**pravidlo scope-freeze**: žádná nová velká doména, dokud A není certifikované). Přesun tieru = 1 řádek v `scope.ts`. ⚠️ **Vizuální „Preview" štítek u třídy B byl SUNDÁN (2026-07-24, vzhled zamítnut vlastníkem)** — registr zůstává jako klasifikace + freeze pravidlo, ale B se v UI vizuálně neodlišuje; `isPreview` = připravené API pro budoucí redesign. **Stav: ✅ klasifikace/registr** (FE-only), vizuál štítku zrušen.
