...
>
> **Snímek k:** 2026-07-24 (**27.2 mobilní core cesty (2026-07-24):** dotyková past taktické mapy opravena — `touch-action:none` z `.viewport` se dědil na scroll kontejnery panelů → tělo token karty i lišta iniciativy se na dotyku scrollují (`pan-y`/`pan-x` + target-guard v `onPointerDown`); dotykové terče akcí sjednoceny ≥40px napříč cestami ①–③ i 24 combat/bestie skiny (husté mřížky pip/box/dot ≥24px), kap. 14 · **25.5 úklid prvního dojmu:** nulové indikátory skryty (chat „Putyka 0" + katalog „0 hráčů"→„zatím bez hráčů"), PWA install prompt za bránou „prožitá hodnota", prefetch nápovědy proti ~3s spinneru, globální **beta marker** doplňuje beta štítek tam, kde patička/banner nejsou — kap. 07 · **25.3 beta rámec:** beta banner — dismissable proužek pod hlavičkou v obou layoutech, jen přihlášený, dismiss v `onboardingStore.dismissed`; odkazuje na hlášení chyb + changelog Vypravěče; patičkový štítek „(beta)" sjednocen do konstanty `betaStage.ts`, kap. 07 · předtím 24.2 ops: nový veřejný `POST /api/csp-report` = sběr porušení CSP bez UI, kap. 08 · **dvě tiše rozbité věci nalezené auditem CSP whitelistu** — náhledy YT videí ve VideosPanelu (kap. 11) a plná paleta emoji pickeru v chatu (kap. 05), obojí blokoval enforce · `ANON_SESSION_TTL` explicitně v configu, kap. 05 · předtím AKJ chráněné záložky: vlastník PC smí inline editovat obsah svých záložek s `ownerEditable` + sjednocený BE merge = fix D-067 PomocnyPJ full-replace data-loss · 11.5 Pavučina: tvorba subjektů z grafu + kontextové menu uzlu + materializace subjektu na reálnou stránku + vyvolávání + nové wiki-like Page-typy Frakce/Organizace/Stát + 15.11 návrhy obsahu hráčů: pending Page se schvalováním PJ + 15.10 správa hráčů v jeskyni: žádosti v kontextu světa + pozvánky `WorldInvite` + přihláška s postavou + 22.4 vitrína + 22.5 sdílení scén + 20.6 přehled využití motivů/skinů · **25.1 (dílčí, 2026-07-24):** in-app hlášení chyb — BE modul `bug-reports` (`POST` veřejný+throttled, admin `GET`/resolve) + Vypravěč „Nahlásit chybu" persona-aware + admin tab Chyby + `/kontakt` e-maily doplněny, kap. 07/08) · **27.1b (2026-07-24):** vazba zlaté cesty ④ scénář→událost→kronika — `GameEvent.scenarioId` („Hraný scénář" v modalu akce) + `TimelineEvent.sourceGameEventId` („Vzešlo z události" v modalu kroniky) + zobrazení chip na kartách, kap. 15) · **Repozitáře:** FE `Projekt-ikaros-FE` (React/TS), BE `Projekt-ikaros/backend` (NestJS).
>
...
>
> 🔁 **Plné přeověření 2026-07-20 (R3 karta 24.5):** všech 16 kapitol projeto proti HEAD obou rep, ~250 driftů opraveno. Datum „Snímek k" výše proto platí pro celou inventuru. **Při dílčí aktualizaci veď datum u konkrétní kapitoly** — jedno globální datum svádí k tomu považovat za ověřené i kapitoly, kterých se měsíc nikdo nedotkl (přesně tak vznikla většina nalezených driftů).
>
> 🚫 **Do inventury NEPATŘÍ stav nasazení.** Věty typu „✅ (BE čeká restart)", „čeká deploy", „necommitnuté" se z kódu nedají ověřit a tiše stárnou: při úklidu 24.5 se kvůli nim **8 položek válelo na 🚧, ačkoli kód byl dávno hotový** — inventura tím podhodnocovala, co platforma umí. Píše se stav KÓDU (✅ / 🚧 / ⚠️); jestli je to nasazené, se zjišťuje zvenku přes `GET /api/health` → `version.sha` (24.1), ne odsud a ne z paměti (`CH-126`).
...
- **Limity tvorby (anti-abuse, D-SEC-GAP-2026-07-11):** velkorysé kumulativní stropy proti creation-floodu (rate-limit brzdí requesty, tohle brzdí dlouhodobý bloat) — **50 světů/účet** (vč. soft-deleted), **2000 stránek/svět** (Pages+Characters sjednoceny), **1000 bestií** (world i user scope), **200 chat skupin** + **500 konverzací/svět**, **50 kalendářů/svět**, **500 map atlasu/svět**, **200 pending naplánovaných zpráv/svět**, **20 náborů/účet**, **200 článků**, **500 obrázků galerie** a **200 diskuzí/účet**. Legitimní hráč je nepotká; při dosažení → **409 `LIMIT_REACHED`** s vlídnou hláškou („ozvi se správci platformy"). Každý limit jde přepsat env proměnnou stejného jména bez rebuildu (čte se lazy per check); guard `assertUnderCreationLimit` je zapojen napříč BE create endpointy (worlds, pages, bestiae, chat skupiny+konverzace, naplánované zprávy, kalendáře, mapy atlasu, nábory, články, galerie, diskuze). Kód: BE `common/limits/creation-limits.ts`.
- **Transakční e-maily přes outbox frontu (D-LAUNCH-GAP „SMTP bez fronty", 2026-07-13):** s SMTP providerem `MailerService.dispatch` mail NEposílá přímo, ale zapíše do Mongo `mail_outbox`; cron `MailOutboxSender` (à 30 s, CronLock v Redis, claim-lease 5 min proti duplicitám) posílá dávky FIFO dle priority. **Denní cap `SMTP_DAILY_CAP`** (default 400 = rezerva pod Gmail ~500) — nad cap odchází už jen vysoká priorita (**reset hesla**), ostatní se odloží na další den + alert. **Retry s backoffem** 1→5→30→240 min; po 5 pokusech `failed` + alert. Fail-safe: selhaný zápis do outboxu → přímé odeslání přes provider. Dev `LogMailerProvider` posílá přímo (cron no-op). „Předáno SMTP" ≠ doručeno — async bounce detekce zůstává dluh. Kód: BE `modules/mailer/{mailer.service,mail-outbox.sender}.ts`.
- **Upload: velikost blobu + disk-cap fallbacku (D-19.2, 2026-07-13):** každý upload obrázku nově vrací **`bytes`** (Cloudinary `bytes`, u disk fallbacku velikost bufferu) — základ pro budoucí měření storage per svět/uživatel (kvóty UM-10; FE zatím `imageBytes` do entit neposílá). Disk fallback (`backend/uploads/` při Cloudinary výpadku) má **cap `UPLOAD_FALLBACK_MAX_MB`** (default 2048; velikost adresáře se skenuje max 1×/min + inkrement po zápisu): nad cap → **503 `STORAGE_FULL`** („Úložiště je dočasně plné") + logError — upload se odmítne, disk sdílený s Mongo přežije (prevence disk incidentů 2026-07). Kód: BE `modules/upload/upload.service.ts:629` (`assertFallbackCapacity`).
...
- **Prerender pro crawlery (15B.1):** veřejné routy se vyhledávačům a sociálním scraperům servírují jako **hotové HTML**; lidé dostávají dnešní SPA beze změny. FE nginx (`default.conf.template`: mapy `$prerender_ua` + `$prerender_path` + `$do_prerender`, interní `location = /__prerender`) podle `User-Agent` (Googlebot, Seznambot, facebookexternalhit, …) **a** whitelistu veřejných cest přepne na **self-hosted headless Chromium sidecar** (`prerender/index.js`, puppeteer-core + alpine Chromium, LRU cache s TTL, graceful fallback na SPA při selhání). SPA signalizuje „obsah domalován" přes `window.__PRERENDER_READY__` (`src/app/PrerenderReady.tsx`, čeká na `useIsFetching()===0`; tvrdý strop 10 s v `main.tsx`). **Leak-safe:** render běží jako anonym — nginx mu maže `Cookie`/`Authorization`, takže privátní svět je BE nedostupný (`OptionalJwtAuthGuard` → 404) a prerender ho fyzicky nezíská. **Whitelist** = `/`, `/podminky`, `/ikaros/{napoveda,novinky,vesmiry,clanky,galerie}`, `/svet/:slug`; auth/citlivé routy (`/chat`, `/reset-password`, `/admin/*`, member-only) se **NEprerenderují**. Per-page `<title>`/description/OG dodává **15B.2** (níže). Sidecar v `docker-compose.yml` (`prerender`, bez host portu, `depends_on` frontend). **Stav: ✅** — kód úplný (render/cache/fallback) a **živě ověřený v produkci** (24.2: bot UA → 200 + `cache: MISS`, 10× souběžně i sekvenčně ⇒ `inflight` guard drží). RAM sidecaru změřena: 419 MiB / 768 MiB limit; `RECYCLE_AFTER` se doladí po druhém měření trendu.
- **SEO meta / sitemap / robots (15B.2):** každá veřejná stránka má **vlastní** `<title>`, meta description, canonical a OG/Twitter kartu přes sdílenou komponentu `<Seo>` (`src/shared/seo/Seo.tsx`) — **bez** knihovny, **nativní React 19 hoisting** `<meta>`/`<link>` do `<head>`; titulek imperativně `document.title` (jinak by hoisting zdvojil statický `<title>` z `index.html`). Zapojeno do 10 veřejných stránek (homepage, vesmíry, články+detail, galerie+detail, nápověda, novinky, podmínky, `/svet/:slug`); description z TipTap přes `metaDescription()` (strip+ořez ~160). **Indexace:** veřejné světy (`public`/`open`) + `Published` články/galerie ano; private/closed svět a nepublikovaný obsah → `noindex` (`<Seo noindex>`). `public/robots.txt` = whitelist (Allow `/`, Disallow `/admin`,`/chat`,`/reset-password`,profil/pošta/diskuze/…) + `Sitemap:` odkaz. **`/sitemap.xml`** = BE modul `seo` (`backend/.../modules/seo`, `GET /api/sitemap.xml`, **leak-safe** — jen veřejné entity, stejné filtry jako přehledy; in-memory cache 1 h); FE nginx proxuje externí `/sitemap.xml` → `https://${BACKEND_HOST}/api/sitemap.xml` (SEO same-host). **Vizuální breadcrumbs** (`src/shared/ui/Breadcrumbs`) na detailech svět/článek/galerie (`Domů › Sekce › název`), datově (`Crumb[]`) využité i pro `BreadcrumbList` JSON-LD (15B.3, níže). **Stav: ✅** — FE+BE kód úplný, testy (FE 11, BE 6) zelené. Kód: `src/shared/seo/{Seo,jsonLd,metaDescription}.*`, BE `modules/seo/seo.controller.ts:13` (`GET /api/sitemap.xml`), `public/robots.txt`. JSON-LD strukturovaná data = **15B.3 (níže)**, sdílecí tlačítka = 15B.6.
- **Strukturovaná data / JSON-LD (15B.3):** veřejné detailové stránky vystaví vyhledávači strojově čitelná data (schema.org) v `<script type="application/ld+json">` → Google může zobrazit náhledový obrázek, datum, drobečky (vyšší proklik). Komponenta `<JsonLd>` + čisté buildery v `src/shared/seo/jsonLd.tsx` (vše v jednom `.tsx` — oddělené `JsonLd.tsx`+`jsonLd.ts` by na Windows case-insensitive FS kolidovalo, viz CH-019). **Schémata:** `Article` (článek; obrázek = 1. `<img>` z TipTap obsahu přes `firstImageSrc`, fallback brand logo — `IkarosArticle` nemá `coverUrl`; tentýž helper napojen i na `og:image` článku v `<Seo>`, takže sdílecí náhled už není jen logo — vyřešená nesrovnalost z 15B.2), `ImageObject` (galerie, vč. `width`/`height`), `CreativeWork` (svět; `creator` z `world.owner.username`), `BreadcrumbList` (reuse `Crumb[]` z breadcrumbs — jeden zdroj pro vizuál i data), `WebSite`+`Organization` (homepage brand). **Gating:** JSON-LD se renderuje jen na `indexable` stránce (stejná podmínka jako `<Seo noindex>` — `Published` obsah / `public`/`open` svět); `serializeJsonLd` escapuje `<` → `<` proti `</script>` breakoutu. **Vědomě vynecháno:** `Person`/`ProfilePage` (veřejné profily jsou v robots `Disallow` + privacy), `DiscussionForumPosting` (diskuze za `requireAuth`+`JwtAuthGuard`, neindexovatelné), `AggregateRating` (Google u `Article` hvězdičky neukazuje, self-serving rating = riziko manual action). **BE beze změny** (čerpá z týchž veřejných entit jako 15B.2). FE `src/shared/seo/jsonLd.tsx`, integrace v `ArticleDetailPage`/`GalleryDetailPage`/`WorldDashboardPage` (jen non-member view)/`DashboardPage`. **Stav: ✅** — FE kód úplný, testy 15/15, build zelený; BE beze změny. Zapojeno v `ArticleDetailPage`/`GalleryDetailPage`/`WorldDashboardPage`/`DashboardPage`. Zbývá jednorázová kontrola Google Rich Results Testem (nástroj třetí strany, ne stav kódu).
- **Přístupnost (a11y — 17.8, první vrstva):** **Globální fokusový prsten** — `:where(a,button,[role=button],input,select,textarea,summary,[tabindex]):focus-visible` v `themes/_shared/reset.css` (0 specificita → per-téma/per-komponentní ringy vyhrají; jen fallback), barva z tokenu `--focus-ring` (`themes/_shared/tokens.css`, téma může přebít). `:focus-visible` → prsten jen při klávesové navigaci. **Sdílený `IconButton`** (`shared/ui/IconButton`) vynucuje `aria-label` na TS úrovni + coarse ≥44×44 (žádné nové ikonové tlačítko bez jména pro čtečku). **Klávesnice:** sdílený `useFocusTrap` (`shared/ui/useFocusTrap.ts`, extrakce z `Modal`) = fokus dovnitř → Tab/Shift+Tab cyklus → navrácení fokusu; napojen na `Modal`, `NotificationCenter`, oba mobilní drawery `IkarosLayout` a mobilní drawer `WorldLayout` (`inert` na zavřený drawer + Escape všude). `KebabMenu` má roving-tabindex (šipky ↑↓ wrap/přeskočí disabled, Home/End). **Statické hlídání:** `eslint-plugin-jsx-a11y` ve `warn` (`eslint.config.js`, 34 pravidel) — a11y regrese už nikdo nezavede potichu. **Hranice / dluh (D-17.8):** `IconButton` má nulovou adopci — ~16 ručních `<button>+ikona` (hlavně chat) ho neadoptovalo, ale VŠECHNA mají `aria-label` (audit: 0 prázdných icon tlačítek) → migrace = kosmetika, ne a11y oprava; focus trap ještě chybí v in-app overlayích (chat rail/sidebar, mapa notebook/token panel — čeká živý test); storybook axe `todo`→`error` blokováno D-033. Kontrast témat hlídá `audit:contrast`. **Stav: 🚧** (většina backlogu ✅ 2026-07-05, WCAG AA certifikace ne — průběžné 🔁). Kód FE `themes/_shared/{reset,tokens}.css`, `shared/ui/{IconButton,useFocusTrap,KebabMenu}/*`, `app/layout/{WorldLayout,IkarosLayout}`, `features/notifications/NotificationCenter.tsx`, `eslint.config.js`. Bez BE zásahu.
- **Scope registr A/B/C + freeze (R3 27.3):** každá viditelná plocha (vstupní bod v UI) je zařazena: **A** = beta core (certifikováno zlatými cestami 27.1, plný support) · **B** = funguje, ale okrajové/hloubkové → nese štítek **„Preview"** (⚗ `<PreviewBadge>`, `shared/ui/PreviewBadge`) · **C** = experimentální/blokované → skryté za feature flagem (nav hide + route `flagGate`). **SSOT** = `src/shared/scope/scope.ts` (`PREVIEW_FEATURES`/`HIDDEN_FEATURES`), lidsky čitelné zrcadlo `docs/scope-registr.md`. **Scoped, ne destruktivní:** in-world nástroje zavedených PJ se neschovávají (C jen pro už-blokované „RPG systémy", 25.8); B = jen vizuální de-emphasis, funkce zůstává. Nová viditelná doména se do bety přidává jako C/B, ne rovnou A (**pravidlo scope-freeze**: žádná nová velká doména, dokud A není certifikované). Přesun tieru = 1 řádek v `scope.ts`. **Stav: ✅** (FE-only). Zapojeno: platform nav (Společná tvorba, Nábory, Voice krčma, 3× camp) + world nav (11 hloubkových nástrojů: Časová osa, Kalendář, Pavučina, Mapa vesmíru, Atlas map, Obchod, Převodník měn, Storyboard, Počasí, Zvuky, Stavitel).
- **Bezpečnostní HTTP hlavičky (14.3):** HTML dokument servíruje **FE nginx** (`default.conf.template` `location /`) — sem patří hlavní **XSS-CSP** (allowlist skriptů/stylů/spojení: self + Cloudflare Turnstile + Google Fonts + Cloudinary + `BACKEND_HOST` přes nginx envsubst), HSTS, Referrer-Policy, Permissions-Policy. **BE** vrací jen JSON/obrázky → `helmet()` jako API hardening (`default-src 'none'`, HSTS, nosniff, `frame-ancestors 'none'`; `crossOriginResourcePolicy:false` aby nerozbil PixiJS `/static/` textury) — `main.ts` za `enableCors`. Inline pre-hydration theme skript povolen přes **SHA-256 hash** + build guard (`scripts/check-csp-hash.mjs`, drift = build fail). **Stav: ✅ enforce** (`CSP_HEADER_NAME` přepínač přepnut; 24.2 doplnila `report-uri` + `report-to` + BE `POST /api/csp-report`). ⚠️ Poučení 24.2: enforce běžel nějakou dobu **naslepo** — bez sběru porušení se tiše rozbily náhledy YT videí a emoji paleta (`CH-125`). Whitelist měnit jen proti `dist/`, ne `src/`. Past: TipTap/PixiJS/dice WASM mohou v report-only vyžádat `style-src 'unsafe-inline'` / `script-src 'wasm-unsafe-eval'`.
