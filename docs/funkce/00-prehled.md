...
- **Přístupnost (a11y — 17.8, první vrstva):** **Globální fokusový prsten** — `:where(a,button,[role=button],input,select,textarea,summary,[tabindex]):focus-visible` v `themes/_shared/reset.css` (0 specificita → per-téma/per-komponentní ringy vyhrají; jen fallback), barva z tokenu `--focus-ring` (`themes/_shared/tokens.css`, téma může přebít). `:focus-visible` → prsten jen při klávesové navigaci. **Sdílený `IconButton`** (`shared/ui/IconButton`) vynucuje `aria-label` na TS úrovni + coarse ≥44×44 (žádné nové ikonové tlačítko bez jména pro čtečku). **Klávesnice:** `Modal` už umí focus trap+Escape+restore (dědí každý dialog); doplněno Escape+`aria-expanded`/`aria-haspopup` na world-nav dropdown (`WorldLayout`), Escape na mobilní drawer (`IkarosLayout`), `role=tablist/tab` na NotifCenter. **Hranice / dluh:** není `eslint-plugin-jsx-a11y` (zaplavil by stovkami nálezů — samostatný pass), migrace všech deníkových listů na `IconButton` je postupná (zatím jen DrdPlus hotspoty dostaly `aria-label`), `KebabMenu` bez šipkové navigace, focus trap zatím jen v `Modal` (drawery/NotifCenter mají jen Escape). Kontrast témat hlídá `audit:contrast`. **Stav: 🚧** (první vrstva ✅, WCAG AA certifikace ne — je to průběžné 🔁). Kód FE `themes/_shared/{reset,tokens}.css`, `shared/ui/IconButton/*`, `app/layout/{WorldLayout,IkarosLayout}`, `features/notifications/NotificationCenter.tsx`. Bez BE zásahu.
