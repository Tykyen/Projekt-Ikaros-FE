...
- **Co jde dělat — sekce formuláře:**
  1. **Základní info** (`BasicInfoSection`) — Název (2–60 zn., **počítadlo znaků** `X/60`), Popis/Identita (max 1000 zn., **počítadlo znaků** `X/1000`). **Adresa/slug je od 2026-07-15 před uživatelem schovaná** (připomínky testerů) — odvozuje se automaticky z názvu (`useWorldSlug`, kebab-case `a-z0-9-`), žádný input se nerenderuje. Live check dostupnosti (`GET /worlds/slug-available`) běží dál na pozadí; jeho výsledek se hlásí u pole **Název** (`taken` → „Svět s tímto názvem už existuje, zvol jiný.", `invalid` → „Z názvu nejde vytvořit adresa — použij aspoň 2 písmena nebo číslice.").
  ...
  8. **Náboženství** (`ReligionSection`, 2.3g) — single select **role náboženství** `religionInfluence` (škála 0–14, default 6 „Organizovaná církev"; dolní půle = sociální vliv sekulární→teokracie, horní = reálná přítomnost božského) + multi-select **14 typů** `religionTypes` (Monoteismus, Polyteismus, Henoteismus, Dualismus, Panteismus, Animismus, Kult předků, Kult přírody, Šamanismus, Mystika, Kult smrti, Vládní/císařský kult, Non-teismus, Temné kulty). Seeduje stránku "Náboženství".
  9. **Motiv** (`ThemeSection`) — výchozí dle žánru, lze ručně přepsat (`themeOverride`).
  10. **Kalendáře** (`CalendarsSection`) — výběr z presetů (default jen Gregorián zaškrtnutý + jako ⭐ výchozí). První zaškrtnutý se stane defaultem; lze přepnout hvězdu.
- **Co se seeduje při založení (BE):**
  ...
  - **Stránky (přes event `world.created`, listener `pages-world-seed`):** seeduje `pravidla`, `magicky-system`, `technologie`, `nabozenstvi` (2.3g), `faq`, `videa`. Obsah: Pravidla = tahák dle systému, Technologie = univerzální škála + zvolený rozsah TÚ, Magický systém = škála + zvolené tradice, **Náboženství** = škála role 0–14 + zvolené typy + osnova „co u víry řešit" (15 bodů). Pro systém `matrix` se Pravidla/Magie/Technologie nahradí kompletní "Pravidlovou knihou" (kapitoly 1–9); **Náboženství se seeduje i pro `matrix`** (kniha ho neřeší).
- **Validace odeslání:** Název OK + slug OK (≥2, ne "taken"/"invalid") + žánr OK + systém OK + slug není "checking". Protože je adresa schovaná, hlásí se nesplněný slug ve výčtu "Vyplň: …" jako **„Název"** (ne "Adresa"). Tlačítko jinak zakázané. Chyby BE: `WORLD_SLUG_TAKEN`/`CONFLICT` → "Adresa už existuje", `WORLD_QUOTA_REACHED` → hláška o limitu 30.
- **Hranice / co neumí:**
...
- **Zvláštnosti:** Po úspěchu redirect na `/svet/<slug>`. Slug se generuje z názvu (`useWorldSlug`) a **od 2026-07-15 už NEJDE ručně přepsat** (input schován, `onSlugChange` se nepředává → dirty flag v hooku zůstává, ale nikdy se neaktivuje = slug vždy auto). `accessMode` default `private` na FE i BE.
- **Stav:** ✅
...
- **Co jde dělat:**
  - **Nástěnka:** filtr strana (Vše / Hledám hráče / Hledám hru), **systém** (dropdown z registru `PLATFORM_SYSTEMS`, 14), **žánr** (dropdown z registru `GENRES`, 11), režim (Online / Naživo), fulltext (nadpis+popis) — vše se skládá **průnikem**, filtruje se klientsky (`filterNabory`). Nabídky jsou **vždy viditelné** (19.3b; dřív se systém odvozoval z dat → na prázdné desce filtr zmizel). Volba bez lístku zešedne, ale zůstává vybratelná; prázdný výsledek nabídne „Zrušit filtry". Pinboard lístků (3/2/1 sloupce responsivně).
  - **Tvorba:** strana (hledám hráče = PJ / hledám hru = hráč); u „hledám hráče" výběr světa → **dědí motiv (`world.themeId`), systém (`resolveSystemId(world.system)`) i žánr (`world.genre`)** (motiv lze přepsat pickerem z 12); nadpis, popis, **systém (select)**, **žánr (select)**, režim (+ město u naživo), počet volných míst; motiv picker 12 s barevným náhledem palety. **Žánr je povinný u „hledám hráče"** (svět ho má povinně), volitelný u „hledám hru"; disabled tlačítko říká přes `title`, co chybí.
...
- **Hranice / co neumí:** `authorIsDeleted` tombstone (Smazaný účet) **BE MVP vynechává** (kvůli `UsersModule` DI) — FE ukáže jméno vždy. Nahlašování jede od 20.1 přes generickou frontu (viz „Nahlásit" výše). `imageUrl` je v entitě, ale **FE upload zatím není** (jen se zobrazí, pokud URL existuje). Expirace 30 dní = jen query filtr `findActive` (ne-expired + `expiresAtUtc` null/budoucí) — **žádný cron**, `status:'expired'` se nikdy sám nenastaví (dnes se skryje jen dle data). Lístek **nemá detailní route** (`/:id`) — jen modal Ozvat se. Filtr nástěnky je **čistě klientský** nad `GET /nabory` — BE query filtr byl v 19.3b vědomě zamítnut (deska potřebuje celý seznam, aby poznala prázdné volby); paginace + facety až s objemem, viz **D-066**. **Nábor nemá „Vlastní" žánr** — svět s custom žánrem nedědí, PJ vybere z 11 (jinak by lístek ve filtru nikdy nebyl). Staré nábory se **volnotextovým** systémem (před 19.3b) v DB zůstávají a do filtru systému nespadnou — najdou se jen fulltextem.
...
- **⚠️ Žánr ≠ motiv (19.3b).** Dvě osy, které se skoro kryjí názvy: **`genre`** = datový fakt (o čem se hraje, 11 hodnot, filtruje se), **`motiv`** = vizuální volba (jak lístek vypadá, 12 tvarů, hráč si vybírá volně). Filtrovat podle motivu = lhát uživateli. Viz spec 19.3 §12.2.
- **⚠️ Systém = canonical id (19.3b).** `Nabor.system` drží **canonical** id (`drdplus`, `coc`) z `PLATFORM_SYSTEMS`, kdežto `world.system` „dlouhá" (`drd-plus`, `call-of-cthulhu`) — proto dědění přes `resolveSystemId`. Dřív bylo pole volnotextové a jeden systém se ve filtru rozpadal na několik zápisů („dnd5e" od PJ vs. „D&D 5e" od hráče). Registry systémů jsou **tři** (`PLATFORM_SYSTEMS` canonical / `RPG_SYSTEMS` dlouhá / `SYSTEM_ALIASES` most) — paritu hlídá `src/shared/rpg/systems.spec.ts`. Enum je navíc **dual-source** FE↔BE (`backend/…/nabory/constants/rpg.ts`) — přidáváš systém/žánr? Změň obě místa (vzor jako theme IDs).
- **Stav:** 🚧 (FE+BE hotové a ověřené — FE build+testy 29/29 vč. 14 paritních, BE typecheck+lint+11 testů; 19.3 nasazeno commitem `1e1f722`, **19.3b filtry systém/žánr necommitnuto a nenasazeno** — čeká FE commit + BE restart + živé ověření; 12 motivů dotažené; zbývá `authorIsDeleted` enrich, upload obrázku, cron expirace)
- **Kód:** FE `src/features/ikaros/pages/NaboryPage.tsx`, `NaborNovaPage.tsx`, lístek `src/features/ikaros/components/NaborListek.tsx` + `naborSkins.css`, hooky `…/api/useNabory.ts`, lib `…/lib/nabory.ts`, typy `src/shared/types/index.ts` (`Nabor`/`NABOR_MOTIVY`), routy `src/app/router.tsx` (`ikaros/nabory` + `/nova`), vstup `src/app/layout/IkarosLayout/IkarosLayout.tsx` (sekce Vesmíry). **Registry (19.3b): `src/shared/rpg/systems.ts` (`PLATFORM_SYSTEMS`, `systemLabel`) + `src/shared/rpg/genres.ts` (`GENRES`, `isKnownGenre`)** — zdroj pravdy, re-exportují je `features/ikaros/bestiar/components/systems.ts` (jako `BESTIE_SYSTEMS`) a `CreateWorldPage/constants/genres.ts`; normalizace `src/features/world/systemId.ts` (`resolveSystemId`); parita `src/shared/rpg/systems.spec.ts`. BE `backend/src/modules/nabory/` (`nabory.controller.ts`, `nabory.service.ts`, `schemas/nabor.schema.ts`, `repositories/nabory.repository.ts`, **`constants/rpg.ts`** = dual-source enum), registrace `backend/src/app.module.ts`.
