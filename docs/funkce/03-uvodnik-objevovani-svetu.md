...
- **Co to je:** Vstupní rozcestník platformy. Uvítací karta + dvě sekce vedle sebe (Akce, Novinky).
...
- **Co jde dělat / co se zobrazí:**
  - `AnonWelcomeSection` — statická uvítací karta ("Vítej v Projektu Ikaros…", podpis "administrátor"). Zobrazuje se **vždy** (anon i přihlášený), navzdory názvu komponenty.
...
- **Hranice / co neumí:** Anon vidí jen uvítací kartu + novinky (sekce Akce mu chybí). Žádná personalizace pro přihlášeného přímo na "/" (Moje světy / nejbližší akce napříč světy jsou jinde — v pravém panelu a v profilu, ne na dashboardu; nápověda a FAQ to popisují jako "rozcestník", ale samotný `DashboardPage.tsx` renderuje jen uvítání + akce + novinky).
- **Zvláštnosti:** "Akce" a "Novinky" zde = **globální platformové** (Ikaros), ne světové. Akce na dashboardu jsou jen pro přihlášené, protože celé `ikaros-events` API je za `JwtAuthGuard`.
- **Stav:** ✅
- **Kód:** FE `src/features/ikaros/pages/DashboardPage/DashboardPage.tsx:15`, sekce `…/sections/AnonWelcomeSection.tsx`, `…/sections/IkarosEventsSection.tsx:14`, `…/sections/PlatformNewsSection.tsx:14`. Router `src/app/router.tsx:147`.
...
- ✅ OPRAVENO 2026-06-18 (přejmenováno na `WelcomeSection`) — **`AnonWelcomeSection` se zobrazuje všem**, ne jen anonymům — název komponenty je zavádějící. Přihlášený vidí stejnou uvítací kartu jako anon (žádná personalizovaná varianta na dashboardu).
- ✅ OPRAVENO 2026-06-18 (FAQ srovnán s realitou) — **Dashboard ≠ popis v nápovědě/FAQ.** FAQ tvrdí, že úvodní stránka po přihlášení je "rozcestník — Moje světy, nejbližší Akce napříč světy a platformové Novinky". Skutečný `DashboardPage` renderuje jen uvítací kartu + globální Akce (max 3) + Novinky (max 3). "Moje světy" a "Moje akce napříč světy" jsou v pravém panelu / profilu, NE na dashboardu. K ověření, zda jde o záměr.
