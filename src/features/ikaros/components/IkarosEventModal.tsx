...
import {
  ...
  useUpdateIkarosEvent,
} from '@/features/ikaros/api/useIkarosEvents';
import { useUploadImage } from '@/features/ikaros/api/useUploadImage';
import {
  createIkarosEventSchema,
  type CreateIkarosEventFormValues,
} from '@/features/ikaros/lib/createIkarosEventSchema';
import type { IkarosEvent } from '@/shared/types';
import s from './IkarosEventModal.module.css';
...
export function IkarosEventModal({ open, onClose, event }: Props) {
  const isEdit = !!event;
  const create = useCreateIkarosEvent();
  const update = useUpdateIkarosEvent();
  ...
  const defaultValues: CreateIkarosEventFormValues = event
    ? {
        title: event.title,
    ...
    : { title: '', date: '', description: '', confirmable: true };
  ...
  const {
    ...
    reset,
    control,
  } = useForm<CreateIkarosEventFormValues>({
    resolver: zodResolver(createIkarosEventSchema),
    mode: 'onBlur',
    defaultValues,
  });
  ...
  function close() {
    onClose();
    reset({ title: '', date: '', description: '', confirmable: true });
    setImageUrl(null);
    setImageError(null);
    setFocal({ x: 50, y: 50 });
    setImageZoom(null);
  ...
  async function onSubmit(values: CreateIkarosEventFormValues) {
    try {
      if (isEdit && event) {
      ...
      toast.error(
        isEdit ? 'Nepodařilo se uložit změny.' : 'Nepodařilo se vytvořit akci.',
      );
    }
  }
    ...
    >
      <form className={s.form} onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className={s.fieldWrap}>
          <label htmlFor="event-date" className={s.label}>
            Datum a čas *
          </label>
          <input
            id="event-date"
        ...
        <div className={s.fieldWrap}>
          <label htmlFor="event-description" className={s.label}>
            Popis (volitelně)
          </label>
          <textarea
        ...
        <Controller
          ...
          render={({ field }) => (
            <label className={s.checkboxRow}>
              ...
              />
              <span>Povolit potvrzení účasti</span>
            </label>
          )}
        />
