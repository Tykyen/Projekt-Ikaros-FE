 ...
 * 'statblock' = balanc statů jednoho systému). Vzor: SpellDiscussion (21.5c);
...
import { usePotionComments } from '../hooks/useLektvary';
import { useAddPotionComment } from '../hooks/useLektvaryMutations';
...
export function PotionDiscussion({
  potionId,
  targetType,
  systemId,
  title,
  ...
  const { data: comments = [], isLoading } = usePotionComments(
    potionId,
    targetType,
    systemId,
  );
  const addComment = useAddPotionComment();
  ...
  const submit = (e: FormEvent) => {
    ...
    if (!content) return;
    addComment.mutate(
      { id: potionId, payload: { targetType, systemId, content } },
      { onSuccess: () => setText('') },
    );
  };
