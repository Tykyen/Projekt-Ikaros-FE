/**
 * 21.5c — dvouúrovňová diskuse komunitního kouzla. Jedna instance = jedna
 * úroveň: `targetType='spell'` (lore, napříč systémy) nebo `'statblock'`
 * (balanc statů jednoho systému). Vzor: BestieDiscussion.
 */
import { useState, type FormEvent } from 'react';
import { useAtomValue } from 'jotai';
import { isAuthenticatedAtom } from '@/shared/store/authStore';
import { useSpellComments } from '../hooks/useKouzla';
import { useAddSpellComment } from '../hooks/useKouzlaMutations';
import s from './SpellDiscussion.module.css';
...
export function SpellDiscussion({
  spellId,
  targetType,
  systemId,
  title,
  ...
  const { data: comments = [], isLoading } = useSpellComments(
    spellId,
    targetType,
    systemId,
  );
  const addComment = useAddSpellComment();
  ...
  const submit = (e: FormEvent) => {
    ...
    if (!content) return;
    addComment.mutate(
      { id: spellId, payload: { targetType, systemId, content } },
      { onSuccess: () => setText('') },
    );
  };
