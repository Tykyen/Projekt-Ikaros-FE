...
const WORLD_PAGES_OK: PageDoc[] = [
  {
    ...
    status: "ok",
    who: "PJ připravuje (hráč vidí jen scénáře, které PJ označil jako sdílené — bez tajných polí)",
    what: 'Nástroj PJ na přípravu a psaní příběhu, uspořádaný do **stromu podle míst a scén** — a zároveň „spustitelná příprava": co si nachystáš, jedním krokem vypustíš do hry. Levý panel = strom: složky (Akt / Kapitola) a scény, libovolná hloubka, **větvení dle voleb hráčů** (podscény s popiskem větve, např. „pokud hráči zradí"). Scény přetáhneš (drag) pro změnu pořadí i zařazení pod jiný uzel; na mobilu přes „Přesunout pod…". Stav scény: koncept / aktivní / volitelná / vyřešená. Pravý panel = **editor scény**: název, text (formátovaný editor s obrázky), **tajná zóna jen pro PJ** (poznámky PJ, cíl a výsledek — hráč nevidí), a **mapa scény** (podkladový obrázek + verze s čísly + vysvětlivky/legenda). U obrázků galerie: otevřít, zkopírovat odkaz a **poslat rovnou do chatu** (vyber kanál) — i s možností **naplánovat odeslání na pozdější čas**. Z mapy scény lze **vytvořit taktickou scénu** jedním tlačítkem. Dole **provázání se světem**: místo scény, libovolné wiki stránky (postavy, novinky…), bestiář (připravený encounter), subjekty Pavučiny a příběhové linky. „Zobrazit v síti" otevře Pavučinu předfiltrovanou na linku scénáře. Scénu lze **uložit jako šablonu do knihovny** (📑) a jindy ji vložit a upravit. Na **taktické mapě** pak tlačítko „Načíst přípravu" jedním krokem vloží předpřipravené postavy a bestie ze scénáře. Příběhové linie se spravují v Pavučině (záložka Linky).',
  },
  {
    path: "/svet/:slug/mapa",
    name: "Mapa vesmíru",
