...
export function CocBestiePanel({
  ...
  const adjustHp = (delta: number): void => {
    if (!canEdit) return;
    const next = maxHp > 0 ? Math.max(0, Math.min(maxHp, currentHp + delta)) : Math.max(0, currentHp + delta);
    // Zelený bar na tokenu čte `systemStats['health.current']` (damageable pole
    // token schématu) — musíme updatovat JEHO, ne jen `currentHp`, jinak bar nereaguje.
    update.mutate({
      tokenId: token.id,
      patch: { currentHp: next, systemStats: sanitize({ ...ss, 'health.current': next }) },
    });
  };
