 ...
 * `token.characterSlug`, prefix `jad_*`, formule z `jad/formulas.ts`.
...
import {
  ...
  type JadFeat,
  type JadWeapon,
} from '@/features/world/pages/CharacterDetailPage/diary-systems/sheets/jad/constants';
import {
  calcMod,
  calcSaveMod,
  calcSkillMod,
...
import styles from './JadCombatPanel.module.css';
...
function classSummary(rows: JadClassRow[]): string {
  return rows
    .filter((r) => r.c)
    .map((r) => `${r.c}${r.l ? ` ${r.l}` : ''}`)
    .join(' / ');
...
export function JadCombatPanel({
  token,
  worldId,
  canEdit,
  onRoll,
  ...
  return (
    <div className={styles.root} data-testid="jad-combat-panel">
