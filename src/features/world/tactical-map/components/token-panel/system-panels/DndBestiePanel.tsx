 ...
 * s chatem přes `DndBestieCombatActions`. Tady mapová specifika: HP ±,
...
} from './DndBestieCombatActions';
import styles from './DndBestiePanel.module.css';
...
export function DndBestiePanel({
  token,
  sceneId,
  worldId,
  canEdit,
  ...
  const sanitize = (stats: Record<string, unknown>): Record<string, unknown> => {
    const sch = systemEntitySchemaRegistry.get('dnd5e', 'token');
    const known = new Set(
  ...
  const doRoll = (req: DndBestieRollReq, initiative = false): void => {
    if (!onMapRoll) return;
