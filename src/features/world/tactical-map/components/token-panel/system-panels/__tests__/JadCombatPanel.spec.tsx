...
import { JadCombatPanel } from '../JadCombatPanel';
...
describe('JadCombatPanel (8.7q fáze A)', () => {
  beforeEach(() => {
    h.mutate.mockClear();
  });
  ...
  it('klik na vlastnost hodí k20 + modifikátor (Síla 16 → +3)', () => {
    ...
    const onRoll = vi.fn();
    render(<JadCombatPanel token={token} {...common} onRoll={onRoll} />);
  ...
  it('ZH u vlastnosti hodí k20 + záchranný bonus (zdatný CON 14, pb 2 → +4)', () => {
    setDiary({ jad_abi_con: '14', jad_save_con: '1', jad_profBonus: '2' });
    const onRoll = vi.fn();
    render(<JadCombatPanel token={token} {...common} onRoll={onRoll} />);
  ...
  it('ukáže jen aktivní dovednosti (se zdatností)', () => {
    setDiary({ jad_skill_Akrobacie: '1' });
    render(<JadCombatPanel token={token} {...common} onRoll={vi.fn()} />);
  ...
  it('bez onRoll / view mode nejsou hody aktivní (disabled)', () => {
    setDiary({ jad_abi_str: '16' });
    render(
      <JadCombatPanel token={token} {...common} canEdit={false} onRoll={vi.fn()} />,
    );
