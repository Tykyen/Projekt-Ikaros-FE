/**
 * 10.2h — paleta nástroje mlhy (PJ). Stejný plovoucí dock vzor jako
 * `EffectsPalette`: toolbar tlačítko 🌫️ → vyjíždějící panel doleva. Aktivní
 * nástroj nese mlhový accent (`#9fb4d4`, chladná stříbrno-modrá — odliší mlhu
 * od teplých/fialových efektů).
 *
 * Obsah: master přepínač „Mlha zap/vyp" (scene.fogEnabled), režim
 * odhalit/zahalit, velikost štětce (1/7/19 hexů), reset. Master vyp →
 * mode+štětec disabled (kreslení nemá smysl bez mlhy).
 *
...
export function FogPalette({
  ...
  const { active, setActive, mode, setMode, brushSize, setBrushSize } = tool;
  const [confirmReset, setConfirmReset] = useState(false);

  const accentStyle = { '--accent': FOG_ACCENT } as React.CSSProperties;
  const brushHexes =
  ...
  return (
    <div className={styles.palette}>
      {active && (
        <div className={styles.panel} style={accentStyle}>
          <div className={styles.panelHeader}>
            <span>🌫️ Mlha války</span>
            <button
              type="button"
      ...
      {/* Toolbar — jediné tlačítko (toggle kreslicího režimu). */}
