 ...
 *
 * Sourozenec mapového `DndBestiePanel`: sdílí prezentační jádro
 * `DndBestieCombatActions` (vlastnosti / útoky / záchrany / schopnosti / kouzla,
...
import {
  ...
  type DndBestieRollReq,
} from '@/features/world/tactical-map/components/token-panel/system-panels/DndBestieCombatActions';
import mapStyles from '@/features/world/tactical-map/components/token-panel/system-panels/DndBestiePanel.module.css';
...
import { calcMod, fmtMod } from '@/features/world/pages/CharacterDetailPage/diary-systems/sheets/dnd5e/formulas';
import { useChatDiaryRoll } from './useChatDiaryRoll';

export type DndChatBestiePatch = {
...
interface Props {
  ...
  systemStats: Record<string, unknown>;
  canEdit: boolean;
  onPatch?: (patch: DndChatBestiePatch) => void;
}
...
export function DndChatBestiePanel({
  worldId,
  channelId,
  rollerName,
  avatarUrl,
  ...
  const doRoll = (req: DndBestieRollReq): void => {
    chatRoll({
      label: req.label,
  ...
  const tokenSchema = systemEntitySchemaRegistry.get('dnd5e', 'token');
...
export default DndChatBestiePanel;
