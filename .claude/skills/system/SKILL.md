...
- ⚠️ **HP bar fill** = `var(--dd-hpfill-grad, …)` v `.hpBarFill`, NIKDY hardcoded `style={{background:hpColor}}` (sablona-skiny §9.15). **HP bar na tokenu** (`resolveCharacterHp`) = přidej `case '<id>'` s REÁLNÝMI klíči `<id>_hpCur/_hpMax` + test proti reálným datům (§9.14); drd2 = jediný v `HP_BAR_DISABLED_SYSTEMS`.
- Ověř → doladění → **brána**.
